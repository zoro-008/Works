...
namespace SML2
{
    public partial class FormErr : Form
    {
        ...
        public void ShowErr(int _iNo)
        {
            ...
            {
                this.Invoke(new VoidDelegate(delegate() { SML.ER.DisplayErrForm(tbName, tbEnum, tbAction, tbErrMsg, pbErr); }));
                this.Invoke(new VoidDelegate(this.Show));
            }
            else
            {
                SML.ER.DisplayErrForm(tbName, tbEnum, tbAction, tbErrMsg, pbErr);
                this.Show();
        ...
        private void pbErr_Paint(object sender, PaintEventArgs e)
        {
            ...
            double dXPos = (double)SML.ER.GetErrConfig(iErrNo).dRectLeft * (double)(dWidthSc + dOfsScale);
            double dYpos = (double)SML.ER.GetErrConfig(iErrNo).dRectTop * (double)(dHeightSc + dOfsScale);

            m_tErrTracker.Rect.X = (int)dXPos;
            m_tErrTracker.Rect.Y = (int)dYpos;
            m_tErrTracker.Rect.Width  = (int)SML.ER.GetErrConfig(iErrNo).dRectWidth  ;
            m_tErrTracker.Rect.Height = (int)SML.ER.GetErrConfig(iErrNo).dRectHeight ;
