...
//      [CategoryAttribute("Motor Para" ), DisplayNameAttribute("Physical No"             )]public int        iPhysicalNo   {get; set;} //실제모터 물리 어드레스.
    ...
    public class CMotrMan
    {
        ...
        public void JogP(int _iMotrNo, double _dVel = 0, double _dAcc = 0, double _dDec = 0)
        {
            if (_iMotrNo < 0 || _iMotrNo >= m_iMaxMotr)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}", _iMotrNo, m_iMaxMotr));
            ...
            if(_dVel > 0) dVel = UnitToPulse(_iMotrNo, _dVel) ;
            if(_dAcc > 0) dAcc = UnitToPulse(_iMotrNo, _dAcc) ;
            if(_dDec > 0) dDec = UnitToPulse(_iMotrNo, _dDec) ;

            Mtr[_iMotrNo].JogP(dVel, dAcc, dDec);
            Log.Trace(GetName(_iMotrNo) , "Jog P");
        }

        public void JogN(int _iMotrNo, double _dVel = 0, double _dAcc = 0, double _dDec = 0)
        {
            if (_iMotrNo < 0 || _iMotrNo >= m_iMaxMotr)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}", _iMotrNo, m_iMaxMotr));
        ...
        public bool GetX(int _iMotrNo,int _iNo)
        {
            ...
            if (_iNo < 0 || _iNo >= 5)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} X is not Between 0 and {1}", _iMotrNo, 5));
                return false;
            }
        ...
        public bool GetY(int _iMotrNo,int _iNo)
        {
            ...
            if (_iNo < 0 || _iNo >= 5)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} Y is not Between 0 and {1}", _iMotrNo, 5));
                return false;
            }
