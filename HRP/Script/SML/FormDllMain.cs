...
namespace SML
{
    ...
    public partial class FormDllMain: Form //UserControl
    {
        ...
        private void tmUpdate_Tick(object sender, EventArgs e)
        {
            ...
            else if (sPageSel == "tpDio")
            {
                for (int i = 0; i < SM.DIO._iMaxIn; i++)
                {
                    ...
                    lvInput.Items[i].SubItems[iIoActValCol].Text = SM.DIO.GetX(i, true) ? "ON" : "OFF";
                    lvInput.Items[i].SubItems[iIoActValCol].BackColor = SM.DIO.GetX(i, true) ? Color.Lime : Color.Silver;
        ...
        private void DispDio()
        {
            for (int i = 0; i < SM.DIO._iMaxIn; i++)
            {
                lvInput.Items[i].SubItems[1 ].Text = string.Format("X{0:000}", SM.DIO.GetInputPara(i).iAdd);
                lvInput.Items[i].SubItems[2 ].Text = string.Format("X{0:X3}", SM.DIO.GetInputPara(i).iAdd);
            ...
            for (int i = 0; i < SM.DIO._iMaxOut; i++)
            {
                ...
                lvOutput.Items[i].SubItems[1 ].Text = string.Format("Y{0:000}", SM.DIO.GetOutputPara(i).iAdd);
                lvOutput.Items[i].SubItems[2 ].Text = string.Format("Y{0:X3}", SM.DIO.GetOutputPara(i).iAdd);
