...
namespace Machine
{
    ...
    public sealed class SEQ
    {
        ...
        static public PreBuffer  PREB = new PreBuffer ((int)pi.PREB + ti.Max);
        static public VisnZone   VSNZ = new VisnZone  ((int)pi.VSNZ + ti.Max);
        static public PostBuffer PSTB = new PostBuffer((int)pi.PSTB + ti.Max);
        ...
        private static void UpdateButton  ()
        {
            ...
            {
                //if (!Interpreter.Instance.ReLoad())
                //{
                //    isRunCon = false;
                //}
                //else
        ...
        public static bool ToStart()      //스타트를 하기 위한 함수.
        {
            ...
            for(int i=0; i < (int)pi.MAX_PART; i++){
                ...
                try
                {
                    int step = (int)EN_SEQ_STEP.ToStart;
