...
namespace Machine
{
    ...
    public class RejectMark : Part
    {
        ...
        public RejectMark(int _iPartId = 0)
        {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //if( DM.ARAY[ri.TRJM].CheckAllStat(cs.Unknown) &&!IO_GetX(xi.TBLE_RJCMPkgDtct)) {ER_SetErr(ei.PKG_Dispr   , "Reject Marking Disappear PKG Error."); return false;}
                //if(!DM.ARAY[ri.TRJM].CheckAllStat(cs.None   ) && IO_GetX(xi.TBLE_RJCMPkgDtct)) {ER_SetErr(ei.PKG_Unknwn  , "Reject Marking Unknown PKG Error.  "); return false;}
                
                bool isCycleReject    = DM.ARAY[ri.TRJM].GetCntStat(cs.NGMark) != 0 && SEQ.TTBL.GetSeqStep() == (int)TurnTable.sc.Idle;//DM.ARAY[ri.BARZ].CheckAllStat(cs.Unknown);
                bool isCycleEnd       = DM.ARAY[ri.TRJM].CheckAllStat(cs.None); //DM.ARAY[ri.BARZ].CheckAllStat(cs.None   );// && IO_GetX(xi.ETC_BendingOut);                              
            ...
            switch (Step.eSeq)
            {
                ...
                case sc.Reject     : if (CycleReject   ()) return false; break;
            }                 
            Trace(sCycle+" End"); 
        ...
        public bool CycleReject()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 10:
                    if(!IO_GetX(xi.TBLE_RJCMPkgDtct)) {
                        ER_SetErr(ei.PKG_Dispr   , "Reject Marking Disappear PKG Error.");
                        Step.iCycle = 0;
                        return true;
                    }
                    MoveCyl(ci.REJM_GrpRtrCwCCw , fb.Bwd);
                    MoveCyl(ci.REJM_RngGrpFwBw  , fb.Bwd);
                    Step.iCycle++;
                    return false;

                case 11:
                    if(!CL_Complete(ci.REJM_GrpRtrCwCCw , fb.Bwd))return false;
                    if(!CL_Complete(ci.REJM_RngGrpFwBw  , fb.Bwd))return false;
                    MoveCyl(ci.REJM_RngGrpFwBw , fb.Fwd);
                    Step.iCycle++;
                    return false;

                case 12:
                    if (!CL_Complete(ci.REJM_RngGrpFwBw, fb.Fwd)) return false;
                    MoveCyl(ci.REJM_GrpRtrCwCCw , fb.Fwd);
                    Step.iCycle++;
                    return false;

                case 13:
                    if (!CL_Complete(ci.REJM_GrpRtrCwCCw, fb.Fwd)) return false;
                    MoveCyl(ci.REJM_RngGrpFwBw , fb.Bwd);
                    Step.iCycle++;
                    return false;
        ...
        {
            if (!CL_Complete(ci.REJM_GrpRtrCwCCw )) return false;
            if (!CL_Complete(ci.REJM_RngGrpFwBw  )) return false;
