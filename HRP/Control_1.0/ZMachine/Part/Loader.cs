    ...
    public class Loader : Part
    {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //int  iTopCoverCnt  = 1 ;
                //int  iBtmCoverCnt  = 
                bool bLdr  = (DM.ARAY[ri.LODR].GetCntStat(cs.Work) == 0 && DM.ARAY[ri.LODR].GetCntStat(cs.Unknown) != 0);
                bool bChk1 = (CL_GetCmd(ci.LODR_PshrRtrCwCCw) == fb.Bwd || CL_GetCmd(ci.LODR_GuideOpCl) == fb.Bwd || CL_GetCmd(ci.LODR_RngJigFwBw) == fb.Bwd);
                bool bChk2 = !DM.ARAY[ri.LODR].CheckAllStat(cs.Empty) && !DM.ARAY[ri.LODR].CheckAllStat(cs.None) ;

                bool isCycleHold =( bLdr || bChk1 ) && bChk2 ;
                bool isCyclePush =  DM.ARAY[ri.LODR].GetCntStat(cs.Work) != 0  && DM.ARAY[ri.PLDR].CheckAllStat(cs.None) && CL_GetCmd(ci.LODR_PshrRtrCwCCw) == fb.Fwd;
                ...
                //Normal Decide Step.
                     if (isCycleHold  ) { DM.ARAY[ri.LODR].Trace(m_iPartId); Step.eSeq = sc.Hold ;  }//Trace(nameof(isCycleHold),bLdr,bChk1,bChk2); }
                else if (isCyclePush  ) { DM.ARAY[ri.LODR].Trace(m_iPartId); Step.eSeq = sc.Push ;  }
            ...
            switch (Step.eSeq)
            {
                ...
                case sc.Hold  : if (!CycleHold  ()) return false; break;
                case sc.Push  : if (!CyclePush  ()) return false; break;
            }
            Trace(sCycle+" End"); 
