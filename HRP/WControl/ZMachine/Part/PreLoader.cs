...
namespace Machine
{
    public class PreLoader : Part
    {
        ...
        public PreLoader(int _iPartId = 0)
        {
        ...
        public bool FindChip(int _iId , out int c, out int r, cs _iChip=cs.RetFail) 
        {
            ...
            //DM.ARAY[_iId].FindFrstColLastRow(ref (cs)_iChip, ref c, ref r);
        ...
        public bool CycleWork()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 13:
                    ...
                    //double dDist = PM.GetValue(mi.MOVE_X, pv.MOVE_XEnd)  - PM.GetValue(mi.MOVE_X, pv.MOVE_XStart);//- dTrgDist;
                    //int    iTrgCnt =(int)( dDist / dTrgDist) ;
                    //double [] dPoses = new double [iTrgCnt] ;
                    //for(int i = 0 ; i < iTrgCnt ; i++)
                    //{
                    //    dPoses[i] = PM.GetValue(mi.MOVE_X, pv.MOVE_XStart) + i * dTrgDist ;
                    //}
                    //
                    //
                    //MT_SetTrgPos(mi.MOVE_X , dPoses , 2 , false , true);
                    //
                    //
        ...
        {
            if (!MT_GetStop(mi.TBL_XMove)) return false;
            if (!MT_GetStop(mi.TBL_YMove)) return false;
