    ...
    public class CMotrMan
    {
        ...
        public bool Init(EN_LAN_SEL _eLanSel,string _sParaFolderPath,EN_MTR_SEL [] _eMotrSels,Enum _eMtr,CDioMan _Dio )
        {
            ...
            for (int i = 0; i < m_iMaxMotr; i++ )
            {
                ...
                else if(_eMotrSels[i] == EN_MTR_SEL.COMI)
                {
                    Mtr[i] = new MotionComizoa.CMotor();
                }
                else
                {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < m_iMaxMotr; i++)
            {
                ...
                //if (GetServo(i) && HomeDone[i] && !GetStop(i)) //커미조아가 겟서보따로해야 해서 뺌
                if (HomeDone[i] && !GetStop(i))
                {
                    //+포지션 리밋 처리.
                ...
                if (!GetStop(i) && !Homming[i]) //커미조아가 겟서보 따로 해야 해서 뺌
                {
                    //+리밋 센서 처리.
