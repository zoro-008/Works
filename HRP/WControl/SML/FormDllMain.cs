...
namespace SML
{
    ...
    public partial class FormDllMain: Form //UserControl
    {
        ...
        private void DispTLampProp(EN_SEQ_STAT _eStat)
        {
            ...
            cbTLNotUsed.Checked = !SM.TWL.GetTLampPara().bUse ;
        }
        ...
        {
            if (!Log.ShowMessageModal("Confirm", "Input Enum을 Name에 복사하시겠습니까?")) return;
            SM.DIO.SetAllInputName();
            if (!Log.ShowMessageModal("Confirm", "Output Enum을 Name에 복사하시겠습니까?")) return;
            SM.DIO.SetAllOutputName();
        }

        private void btAdd_Click(object sender, EventArgs e)
        {
            if (!Log.ShowMessageModal("Confirm", "Input No을 Add에 복사하시겠습니까?")) return;
            SM.DIO.SetAllInputAdd();
            if (!Log.ShowMessageModal("Confirm", "Output No을 Add에 복사하시겠습니까?")) return;
            SM.DIO.SetAllOutputAdd();
        }

        private void FormDllMain_KeyDown(object sender, KeyEventArgs e)
        {
            if(e.Modifiers == Keys.Alt && e.KeyCode == Keys.A)
            {
                if(tcMain.SelectedTab.Name == tpError    .Name) btErrApply  .PerformClick();
                if(tcMain.SelectedTab.Name == tpDio      .Name) btDioApply  .PerformClick();
                if(tcMain.SelectedTab.Name == tpAio      .Name) btAioApply  .PerformClick();
                if(tcMain.SelectedTab.Name == tpTowerLamp.Name) btTLampApply.PerformClick();
                if(tcMain.SelectedTab.Name == tpCylinder .Name) btCylApply  .PerformClick();
            }
            if(e.Modifiers == Keys.Alt && e.KeyCode == Keys.S)
            {
                if(tcMain.SelectedTab.Name == tpError    .Name) btSaveErr  .PerformClick();
                if(tcMain.SelectedTab.Name == tpDio      .Name) btSaveIO   .PerformClick();
                if(tcMain.SelectedTab.Name == tpAio      .Name) btSaveAIO  .PerformClick();
                if(tcMain.SelectedTab.Name == tpTowerLamp.Name) btSaveTLamp.PerformClick();
                if(tcMain.SelectedTab.Name == tpCylinder .Name) btSaveCyl  .PerformClick();
                if(tcMain.SelectedTab.Name == tpMotor    .Name) btSaveMotr .PerformClick();
            }
