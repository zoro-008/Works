...
namespace SML
{
    ...
    {
        //public FormErr FrmErr; //Windows Form 사용
        View.Error FrmErr; //WPF 사용
        ...
        public bool Init(EN_LAN_SEL _eLanSel,string _sParaFolderPath,Enum _eErr,bool _bUseErrPic)
        {
            ...
            //FrmErr = new FormErr(); //Windows Form 사용
            FrmErr = new View.Error(); //WPF 사용
        ...
        public void SetErr(int _iNo , string _sMsg="",[CallerMemberName] string memberName = "",[CallerLineNumber] int sourceLineNumber = 0, [CallerFilePath] string sourceFilePath = "")
        {
            ...
            Log.Trace(String.Format("ERROR({0:000})", _iNo) + "_" + String.Format("FUNC : {0} LINE : {1} SOURCE PATH : ", memberName, sourceLineNumber) + sourceFilePath, ForContext.Sts);
        }
        ...
        public void WPF_DisplayErrForm(TextBox _tbName, TextBox _tbEnum, TextBox _tbAction, TextBox _tbErrMsg, MediaElement _pbErr, Canvas _Tracker, double? _iPixelWidth, double? _iPixelHeight)
        {
            //Local Var.
            string Temp   ;
            int iErrNo    ;
            ...
            _Tracker.Children.Clear();

            iPicWidth  = (int)_Tracker.Width ;
            iPicHeight = (int)_Tracker.Height;

            iErrNo = GetLastErr();


            //에러 사진 디스플레이
            //System.Windows.Controls.MediaElement ErrImage = new System.Windows.Controls.MediaElement();
            //System.Windows.Controls.Image ErrImage = new System.Windows.Controls.Image();
            if (m_aErr[iErrNo].Para.sImgPath != "") 
            {
                fi = new FileInfo(m_aErr[iErrNo].Para.sImgPath);
                _pbErr.UnloadedBehavior = MediaState.Manual;
                _pbErr.LoadedBehavior = MediaState.Play;
                _pbErr.Source = new Uri(m_aErr[iErrNo].Para.sImgPath);
                //ErrImage.Source = new BitmapImage(new Uri(m_aErr[iErrNo].Para.sImgPath));
                _pbErr.Stretch = System.Windows.Media.Stretch.Uniform;
                if (_iPixelWidth.HasValue)
                {
                    _pbErr.Width = (int)_iPixelWidth.Value;
                }
                if (_iPixelHeight.HasValue)
                {
                    _pbErr.Height = (int)_iPixelHeight.Value;
                }
            }

            //에러 사진의 트랙커 디스플레이
            double dOfsScale = 0.0;
            System.Windows.Shapes.Rectangle Tracker = new System.Windows.Shapes.Rectangle();
            
            if (_pbErr != null)
            {
                int iImgErrWidth  = SM.ERR.GetPanelSize().iWidth ; //Util 폼에서 ErrImage 픽쳐박스 폭 사이즈
                int iImgErrHeight = SM.ERR.GetPanelSize().iHeight; //Util 폼에서 ErrImage 픽쳐박스 높이 사이즈
                
                double dWidthSc  = _Tracker.Width  / (double)iImgErrWidth ; //Util 폼과 Form Err의 에러 사진의 스케일
                double dHeightSc = _Tracker.Height / (double)iImgErrHeight; //Util 폼과 Form Err의 에러 사진의 스케일
                
                double dXPos = SM.ERR.GetErrConfig(iErrNo).dRectLeft * (dWidthSc  + dOfsScale);
                double dYpos = SM.ERR.GetErrConfig(iErrNo).dRectTop  * (dHeightSc + dOfsScale);
                
                Tracker.Width  = SM.ERR.GetErrConfig(iErrNo).dRectWidth  * dWidthSc ;
                Tracker.Height = SM.ERR.GetErrConfig(iErrNo).dRectHeight * dHeightSc;
                Tracker.Stroke = System.Windows.Media.Brushes.Red;
                Tracker.StrokeThickness = 3;
                Canvas.SetLeft(Tracker, dXPos);
                Canvas.SetTop (Tracker, dYpos);
            } 
            ...
            _tbAction.Clear();
            foreach (var i in sSplit) _tbAction.Text += i.ToString().Trim() + "\r\n";

            Temp = m_aErr[iErrNo].Para.sImgPath;
            if (m_aErr[iErrNo].Para.sImgPath == "" || !fi.Exists)
            {
                _pbErr  .Visibility = System.Windows.Visibility.Hidden;
                _Tracker.Visibility = System.Windows.Visibility.Hidden;
            }
            else if (fi.Exists)
            {
                _pbErr  .Visibility = System.Windows.Visibility.Visible;
                _Tracker.Visibility = System.Windows.Visibility.Visible;
                //_Tracker.Children.Add(ErrImage);// = Image.FromFile(Temp);
                _Tracker.Children.Add(Tracker);
            }
        }

        //Win Form에서 사용하는 함수
        public void DisplayErrForm(System.Windows.Forms.TextBox _tbName, System.Windows.Forms.TextBox _tbEnum, System.Windows.Forms.TextBox _tbAction, System.Windows.Forms.TextBox _tbErrMsg, System.Windows.Forms.PictureBox _pbErr)
        {
            //Local Var.
            string Temp, Str;
            int iRect;
            ...
            {
                _pbErr.Visible = true;
                _pbErr.Image = System.Drawing.Image.FromFile(Temp);
            }
