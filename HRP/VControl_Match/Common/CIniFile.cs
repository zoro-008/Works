...
namespace COMMON
{
    public class CIniFile
    {
        ...
        public static T StrToEnumDef<T>(string _sVal, T _eDef) //where T : struct
        {
            T eRet = default(T);
            try 
            {
                eRet = (T)Enum.Parse(typeof(T) , _sVal) ;
            }
            catch(Exception _e)
            {
                eRet = default(T);
        ...
        //public static T StrToEnumDefZero<T>(string _sVal, T _eDef) //where T : struct
        //{
        //    T eRet = default(T);
        //    try 
        //    {
    ...
    public class CAutoIniFile
    {
        public static bool LoadStruct<T>(string _sFilePath, string _sSection , ref T _oStruct)
        {
            ...
            //    else if(f[i].FieldType == typeof(uint  ))f[i].SetValueDirect(__makeref(_oStruct), CIniFile.StrToUintDef  (sValue , 0    ));
            ...
            PropertyInfo[] p = type.GetProperties(BindingFlags.Public|BindingFlags.NonPublic | BindingFlags.Instance);
            for(int i = 0 ; i < p.Length ; i++){
                sKey = p[i].Name ;
                Ini.Load(_sSection , sKey , out sValue);
                     if(p[i].PropertyType          == typeof(bool  ))p[i].SetValue(_oStruct, CIniFile.StrToBoolDef  (sValue , false));
                else if(p[i].PropertyType          == typeof(int   ))p[i].SetValue(_oStruct, CIniFile.StrToIntDef   (sValue , 0    ));
                else if(p[i].PropertyType          == typeof(uint  ))p[i].SetValue(_oStruct, CIniFile.StrToUintDef  (sValue , 0    ));
                else if(p[i].PropertyType          == typeof(double))p[i].SetValue(_oStruct, CIniFile.StrToDoubleDef(sValue , 0.0  ));
                else if(p[i].PropertyType          == typeof(string))p[i].SetValue(_oStruct, sValue                                 );
                else if(p[i].PropertyType.BaseType == typeof(Enum  ))p[i].SetValue(_oStruct, Enum.Parse(p[i].PropertyType , sValue));
                     
                     //여기..
                     //else if(p[i].PropertyType.IsEnum           )p[i].SetValue(_oStruct, CIniFile.StrToEnumDef  (sValue , Convert.ChangeType(0,p[i].PropertyType)));
        ...
        //{
        //    T eRet ;
        //    if (Enum.TryParse<T>(_sVal, out eRet)) return eRet;
        //    return _eDef;
        //}
        ...
        public static bool SaveStruct<T>(string _sFilePath, string _sSection , ref T _oStruct)
        {
            ...
            for(int i = 0 ; i < p.Length ; i++){
                ...
                try
                {
                    sValue = p[i].GetValue(_oStruct).ToString();  
                }
                catch(Exception _e)
                {
