...
namespace VDll.Pakage
{
    class PEdge : CPkgObject , IPkg
    {
        ...
        public PEdge(string _sName , int _iVisionID):base(_sName , _iVisionID)
        {
        ...
        public bool Init()
        {
            ...
            AlgPara = Activator.CreateInstance(typeof(Algorithm.CEdge.CUPara));
        ...
        public bool Run(TRunPara _tPara , ref TRunRet _tRet)
        {
            ...
            if(lsTracker[0] == null) return false;
            ...
            Mat Img = MstPara.PkgImage.GetTrainImage();
            ...
            //bool IsPat = Math.Abs((int)lsTracker[0].GetPointX(iEnd) - (int)lsTracker[0].GetPointX(iStart)) >= 
            //             Math.Abs((int)lsTracker[0].GetPointY(iEnd) - (int)lsTracker[0].GetPointY(iStart));
            lock (GV.ImageLock[iVisionID])
            {
                lsTracker[0].visible = true;
                
                Algorithm.CEdge.CUPara  EdgePara = AlgPara as Algorithm.CEdge.CUPara;
                Algorithm.CEdge.TRslt   EdgeRslt = new Algorithm.CEdge.TRslt();
                
                //int StartX  = IsPat ? (int)lsTracker[0].GetPointX(iStart) : (int)lsTracker[0].GetPointY(iStart);
                //int EndX    = IsPat ? (int)lsTracker[0].GetPointX(iEnd  ) : (int)lsTracker[0].GetPointY(iEnd  );
                //int StartY  = IsPat ? (int)lsTracker[0].GetPointY(iStart) : (int)lsTracker[0].GetPointX(iStart);
                //int EndY    = IsPat ? (int)lsTracker[0].GetPointY(iEnd  ) : (int)lsTracker[0].GetPointX(iEnd  );
                int StartX  = (int)lsTracker[0].GetPointX(iStart);
                int EndX    = (int)lsTracker[0].GetPointX(iEnd  );
                int StartY  = (int)lsTracker[0].GetPointY(iStart);
                int EndY    = (int)lsTracker[0].GetPointY(iEnd  );
                
                if (!Algorithm.CEdge.FindEdge(Img, StartX ,EndX,
                                                   StartY ,EndY, EdgePara, ref EdgeRslt))
                {
                    sError = EdgeRslt.sError;
                    //return false ;
                }
            ...
            //lsTracker[0].trackerType = CTracker.ETrackerType.ttLine;
            //lsTracker[0].visible = true;
            //
            //Algorithm.CEdge.CUPara  EdgePara = AlgPara as Algorithm.CEdge.CUPara;
            //Algorithm.CEdge.TRslt   EdgeRslt = new Algorithm.CEdge.TRslt();
            ...
            //int iEnd   = (int)CTracker.EPointId.piLineEnd  ;
            //
            //if (!Algorithm.CEdge.FindEdge(Img, (int)lsTracker[0].GetPointX(iStart) ,(int)lsTracker[0].GetPointX(iEnd  ),
            //                                   (int)lsTracker[0].GetPointY(iStart) ,(int)lsTracker[0].GetPointY(iEnd  ), EdgePara, ref EdgeRslt))
            //{
            //    sError = EdgeRslt.sError;
            //    //return false ;
            //}
...
//                Algorithm.CEdge.CUPara  EdgePara = AlgPara as Algorithm.CEdge.CUPara;
//                Algorithm.CEdge.TRslt[] EdgeRslt = new Algorithm.CEdge.TRslt[iWidth];
...
//                    {
//                        if (!Algorithm.CEdge.FindEdge(Img, 10, 100, 3, 3, EdgePara, ref EdgeRslt[x]))
//                        {
//                            sError = EdgeRslt[x].sError;
//                            //return false ;
//                        }
//                        //Rslt.Peaks[x].X = Rect.Left + x;
//                        //if (EdgeRslt[x].fPos != 0)
//                        //{
//                        //    Rslt.Peaks[x].Y = Rect.Top + EdgeRslt[x].fPos;             
//                        //}
//                        //else
//                        //{
//                        //    Rslt.Peaks[x].Y = 0;
...
//                        //float fHeightValue = (fTrackerHeight - EdgeRslt[x].fPos) * fTrackerRatio;
...
//            _g.DrawString("MultiProfile WorkTime : " + Rslt.WorkTimems.ToString() , new Font("헤움봄비152", 20), Brushes.Yellow, 0, 0);
        ...
        public bool PaintTrain (Graphics _g)
        {
            try 
            {
                ...
                Mat TrainImg = MstPara.PkgImage.GetTrainImage();
                lock (GV.ImageLock[iVisionID])
                {
                    if (tScaleOffset.fScaleX == 0 && tScaleOffset.fScaleY == 0)
                    {
                        SetFitScaleOffset(_g.ClipBounds.Width, _g.ClipBounds.Height, TrainImg.Width, TrainImg.Height);
                    }
...
//                }
                ...
                lock (GV.ImageLock[iVisionID])//lock(GV.ImageLock[iVisionID]) //
                {                                
                    using(Pen pen = new Pen(Color.Yellow , 1 ))
                    {
                        foreach( CTracker Tracker in lsTracker)
