...
namespace VDll.Pakage
{
    public struct TProp
    {
        public bool bImgGrabber ; //카메라 같은 이미지 촬영.        
    ...
    public struct TPos //이거 나중에 클래스화 해야함. 함수를 만들어 포지션 의 회전위치 리턴.
    {
    ...
    public struct TScaleOffset
    {
        ...
        public TScaleOffset (float _fScaleX , float _fScaleY , float _fOffsetX , float _fOffsetY)
        {
            fScaleX  = _fScaleX  ;
            fScaleY  = _fScaleY  ;
            fOffsetX = _fOffsetX ;
    ...
    public static class MatExtension
    {
        ...
        //나중에 이거 없애고 _g.TranslateTransform() _g.ScaleTransform() 이용하여 해보자.
    ...
    {
        protected TScaleOffset tScaleOffset = new TScaleOffset(1,1,0,0) ;
        ...
        //현재 판넬과 이미지의 크기를 계산해서 오프셑과스케일세팅.
    ...
    public interface IPkg
    {
        ...
        bool           Paint      (Graphics _g , bool _bTrain);// , TScaleOffset _ScaleOffset);//매개변수 화면 핸들 혹은 ImageBox포인터.
        bool           PaintScOf  (Graphics _g , bool _bTrain , TScaleOffset _ScaleOffset);// , TScaleOffset _ScaleOffset);//매개변수 화면 핸들 혹은 ImageBox포인터.
