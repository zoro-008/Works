...
namespace VDll.Pakage
{
    ...
    {
        readonly TProp Prop = new TProp{bImgGrabber = true  ,
                                        bHaveDialog = true  ,
                                        bHaveImage  = true  ,
                                        bHavePosRef = false };
        ...
        Mat CapturedImage  = new Mat(); //트레인시에 카메라 페키지에서 Train을 누르면 ActiveImage => TrainImage
        ...
        public Mat GetImage ()
        {
            if(GV.Visions[iVisionID].VisnStats.Trainings) return CapturedImage ;
            return ActiveImage ;
        }
        ...
        public bool Paint(Graphics _g , bool _bTrain)
        {
            if(tScaleOffset.fScaleX == 0 && tScaleOffset.fScaleY ==0)
            {
                if (_bTrain)
                {
                    SetFitScaleOffset(_g.ClipBounds.Width , _g.ClipBounds.Height , CapturedImage.Width , CapturedImage.Height);
                }
                else
                {
        ...
        public bool PaintScOf  (Graphics _g , bool _bTrain , TScaleOffset _ScaleOffset)
        {
            if(_bTrain)
            {
                ...
                try
                {
                    CapturedImage.Paint(_g, _ScaleOffset);
                }
                catch (Exception _e)
                {
                    sError = _e.ToString();
            ...
            else
            {
                ...
                try
                {
                    ActiveImage.Paint(_g, _ScaleOffset);
                }
                catch (Exception _e)
                {
                    sError = _e.ToString();
        ...
        public bool LoadSave (bool _bLoad , string _sPath)
        {
            ...
            if (_bLoad)
            {
                ...
                if(File.Exists(TrainImgPath ))
                {
                    try
                    {
                        CapturedImage  = CvInvoke.Imread(TrainImgPath , ImreadModes.Unchanged );
                    }
                    catch(Exception _e)
                    {
                        sError = _e.ToString() ;
            ...
            else
            {
                ...
                if(CapturedImage  != null)CapturedImage .Save(TrainImgPath );
                if(ActiveImage != null)
                {
                    lock (GV.ImageLock[iVisionID])
                    {
        ...
        public bool LoadTrainImage(string _sPath)
        {
            ...
            { //여기랑 Display랑 서로 라킹 안걸면 서로 침해함.
                ...
                CapturedImage = new Mat(_sPath, ImreadModes.Unchanged);
            }
        ...
        public bool CaptureImg()
        {
            ...
            lock (GV.ImageLock[iVisionID])//lock(ActiveImage) 
            {
                ...
                CapturedImage = ActiveImage.Clone();
            }
