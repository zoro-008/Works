    ...
    public class Unloader : Part
    {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //Normal Decide Step.
                     if (isSupply ) { DM.ARAY[ri.ULDR].Trace(m_sPartName); Step.eSeq = sc.Supply; }
                else if (isPick   ) { DM.ARAY[ri.ULDR].Trace(m_sPartName); Step.eSeq = sc.Pick  ; }
                else if (isWork   ) { DM.ARAY[ri.ULDR].Trace(m_sPartName); Step.eSeq = sc.Work  ; }
                else if (isDrop   ) { DM.ARAY[ri.ULDR].Trace(m_sPartName); Step.eSeq = sc.Drop  ; }
