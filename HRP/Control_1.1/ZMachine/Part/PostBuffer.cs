    ...
    public class PostBuffer : Part
    {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //Normal Decide Step.
                     if (isWait) { DM.ARAY[ri.PSTB].Trace(m_sPartName); Step.eSeq = sc.Wait; }
                else if (isIn  ) { DM.ARAY[ri.PSTB].Trace(m_sPartName); Step.eSeq = sc.In  ; }
                else if (isWork) { DM.ARAY[ri.PSTB].Trace(m_sPartName); Step.eSeq = sc.Work; }
                else if (isOut ) { DM.ARAY[ri.PSTB].Trace(m_sPartName); Step.eSeq = sc.Out ; }
