...
namespace Machine
{
    public class PostLoader : Part
    {
        ...
        public PostLoader(int _iPartId = 0)
        {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                bool isCycleWork   = DM.ARAY[ri.TLDR].CheckAllStat(cs.None) && DM.ARAY[ri.PLDR].GetCntStat(cs.Unknown) != 0 && SEQ.TTBL.GetSeqStep() == (int)TurnTable.sc.Idle;
                bool isCycleRvrs   = CL_Complete(ci.LODR_RngGrpFwBw , fb.Bwd) && MT_GetStopPos(mi.LODR_XPckr , pv.LODR_XPckrPlce) && CL_Complete(ci.LODR_GrpRtrCwCCw , fb.Fwd);

                bool isCycleEnd    = DM.ARAY[ri.PLDR].CheckAllStat(cs.None) ; //DM.ARAY[ri.SPLR].CheckAllStat(cs.None) && (!IO_GetX(xi.LODR_TrayDtct) || OM.CmnOptn.bGoldenTray);
                                   
                
                if (ER_IsErr()) return false;
                //Normal Decide Step.
                     if (isCycleWork  ) { Step.eSeq = sc.Work ;  }
                else if (isCycleRvrs  ) { Step.eSeq = sc.Rvrs ;  }
            ...
            switch (Step.eSeq)
            {
                ...
                case (sc.Rvrs  ): if (CycleRvrs  ()) { Trace(sCycle+" End"); m_CycleTime[(int)Step.eSeq].End(); Step.eSeq = sc.Idle; } return false;
            }
        }
        //인터페이스 상속 끝.==================================================
        #endregion
        ...
        public bool CycleWork()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 11:
                    ...
                    if (!MT_GetStopPos(mi.LODR_XPckr, pv.LODR_XPckrPickRtt)) return false;
        ...
        public bool CheckSafe(ci _eActr, fb _eFwd, bool _bChecked = false)
        {
            ...
            {
                //if (CL_GetCmd(ci.LODR_RngJigFwBw) != fb.Fwd) 
                //{ 
