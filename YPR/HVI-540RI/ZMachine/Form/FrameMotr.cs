...
namespace Machine
{
    ...
    public partial class FraMotr : Form
    {
        ...
        private void btNeg_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (m_eId == mi.LODR_YIndx) bRet = SEQ.LODR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.LODR_XPshr) bRet = SEQ.LODR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.TBLE_TTble) bRet = SEQ.TTBL.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.LODR_XPckr) bRet = SEQ.LODR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.VISN_ZGrpr) bRet = SEQ.VISN.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.ULDR_ZPckr) bRet = SEQ.ULDR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.ULDR_XGrpr) bRet = SEQ.ULDR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.ULDR_YIndx) bRet = SEQ.ULDR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.ULDR_ZNzzl) bRet = SEQ.ULDR.CheckSafe((mi)m_eId, (pv)uPstnNo);
            if (m_eId == mi.ULDR_XNzzl) bRet = SEQ.ULDR.CheckSafe((mi)m_eId, (pv)uPstnNo);
