...
namespace Machine
{
    ...
    public class VC
    {
        ...
        public struct COPYDATASTRUCTSUB
        {
            public IntPtr dwData;
            public int cbData;
            //[MarshalAs(UnmanagedType.LPStr)]
        ...
        static private void SendMsg(mc _MsgId, string _sSubMsg = "")
        {
            ...
            RecvData.dwData = (IntPtr)mc.NONE;
            RecvData.cbData = 0;
            RecvData.lpData = "";
        ...
        static private bool CycleSendReady()
        {
            ...
            if (m_tmTimeOut.OnDelay(SendStat.iStep != 0 && SendStat.iStep == SendStat.iPreStep, 500))
            {
                ...
                SM.ER.SetErrMsg((int)ei.VSN_ComErr, SendStat.sErrMsg);
                SendStat.iStep = 0;
                return true;
            }
            ...
            switch (SendStat.iStep)
            {
                ...
                case 11:
                    if (RecvData.dwData == (IntPtr)mc.NONE) return false;//"OK"
        ...
        static private bool CycleSendChange()
        {
            ...
            switch (SendStat.iStep)
            {
                ...
                case 11:
                    if (RecvData.dwData == (IntPtr)mc.NONE) return false;
                    SendStat.sMsg = RecvData.lpData;
                    Log.Trace("Vision JobChanged",RecvData.lpData);
                    SendStat.iStep = 0;
                    return true;
        ...
        static public void SetReceivedMsg(COPYDATASTRUCT _DataMsg)
        {
            try
            {
                RecvMstView = _DataMsg;
                Log.Trace("VisnRcv", _DataMsg.lpData);
            }
            catch(Exception _e)
            {
            ...
            if (SendStat.MsgId == sm.None && RecvData.dwData == (IntPtr)mc.RESULT)
            {
                ...
                RecvStat    .MsgId  = (mc)RecvMstView.dwData;
                RecvStat    .bEnded = false;
                RecvData = RecvMstView;
                //RecvMstView.lpData  = RecvMstView.lpData;
            }
             
            else
            {
                if(SendStat.MsgId == sm.Insp)
                {

                    //System.IO.StreamReader a = new System.IO.StreamReader("C:\\Data\\Result.dat");
                    //RecvMstView.lpData = a.ReadLine();
