...
namespace Machine
{
    public partial class FormDevice : Form
    {
        ...
        private void tmUpdate_Tick(object sender, EventArgs e)
        {
            ...
            if (SEQ._iSeqStat == EN_SEQ_STAT.ssStop || SEQ._iSeqStat == EN_SEQ_STAT.ssError) 
            {
            ...
            switch (iLevel)
            {
                case (int)EN_LEVEL.lvOperator: btSetting.Enabled = false; break;
                case (int)EN_LEVEL.lvEngineer: btSetting.Enabled = true; break;
                case (int)EN_LEVEL.lvMaster: btSetting.Enabled = true; break;
                default: break;
            }
        ...
        private void btDownload_Click(object sender, EventArgs e)             //HRM-930B 참고
        {
            ...
            //bool bAllArayNone = DM.ARAY[(int)ri.LENS].CheckAllStat(cs.None);

            if (LOT.GetLotOpen())
            {
                //if (!bAllArayNone)
                //{
                    Log.ShowMessage("Error", "Can not Change jobfile while LotOpened.");
                    return;
                //}
            }
            ...
            {
                Log.ShowMessage( "ERROR", "Same as current jobfile");
                return;
            }
            ...
            VC.SendVisnMsg(VC.sm.Change, OM.GetCrntDev());
            //어차피 답변 먼저 보내고 잡파일 변환 하는듯...
            //CDelayTimer TimeOut = new CDelayTimer();
            //TimeOut.Clear();
            //while (!VC.IsRecvMsg())
            //{ //메세지 다 주고 받을때까지 기다림.
            //    Thread.Sleep  (1);
            //    if (TimeOut.OnDelay(true, 10000))
            //    {
            //        Log.ShowMessage("Error", "Vision JobChange TimeOut!");
            //        break;
            //    }
            //}
        }
        ...
        private void btNew_Click(object sender, EventArgs e)
        {
            ...
            if (tbToName.Text == "")
            {  
                Log.ShowMessage("ERROR", "Input job name");
                return;
            }

            if (di.Exists == true)
            {
                Log.ShowMessage("Warning", "There is same jobfile name.");
                return;
            }
        ...
        private void btCopy_Click(object sender, EventArgs e)
        {
            if (tbFromName.Text == "")
            {   
                Log.ShowMessage("Error", "Please select jobfile");
                return;
            }
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                Log.ShowMessage("Warning", "the jobfile is using now" );
                return;
            }
