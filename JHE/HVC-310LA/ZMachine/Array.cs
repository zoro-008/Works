...
namespace Machine
{
    public struct TChip
    {
        public cs  Stat;
        public int iTag;
        public double dDataT; //일단 홀더 검사각도로 씀.
        public double dDataX;
        public double dDataY;
        //public string sData ;
        //double dDataX; 나중에 만들어야 함
        //double dDataY;
        ...
        public void Load(CConfig Config , bool IsLoad)
        {
            ...
            if(IsLoad) {
                ...
                try
                {
                    ...
                    for (int r = 0; r < m_iMaxRow; r++)
                    {
                        sIndex = string.Format("RowDataT{0:000}", r);
                        Config.GetValue(sArayName, sIndex, out sRowData);
                        string[] sDatas = new string[m_iMaxCol];
                        sDatas = sRowData.Split(',');
                        for (int c = 0; c < m_iMaxCol; c++)
                        {
                            if (sDatas.Length <= c) break;
                            dTemp = double.Parse(sDatas[c]);
                            m_tChips[c, r].dDataT = dTemp; 
                        }
                    }
                    for (int r = 0; r < m_iMaxRow; r++)
                    {
                        sIndex = string.Format("RowDataX{0:000}", r);
                        Config.GetValue(sArayName, sIndex, out sRowData);
                        string[] sDatas = new string[m_iMaxCol];
                        sDatas = sRowData.Split(',');
                        for (int c = 0; c < m_iMaxCol; c++)
                        {
                            if (sDatas.Length <= c) break;
                            dTemp = double.Parse(sDatas[c]);
                            m_tChips[c, r].dDataX = dTemp;
                        }
                    }
                    for (int r = 0; r < m_iMaxRow; r++)
                    {
                        sIndex = string.Format("RowDataY{0:000}", r);
                        Config.GetValue(sArayName, sIndex, out sRowData);
                        string[] sDatas = new string[m_iMaxCol];
                        sDatas = sRowData.Split(',');
                        for (int c = 0; c < m_iMaxCol; c++)
                ...
                for (int r = 0; r < m_iMaxRow; r++)
                {
                    ...
                    for (int c = 0; c < m_iMaxCol; c++)
                    {
                        dTemp = m_tChips[c, r].dDataT; 
                        sTemp = string.Format("{0:0.0000}", dTemp);
                        sRowData += sTemp + ",";
                    }
                    Config.SetValue(sArayName, sIndex, sRowData);
        ...
        public void pbArray_MouseUp(object sender, MouseEventArgs e) //TObject *Sender,
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                if (ClickStat.iSelX1 == ClickStat.iSelX2 && ClickStat.iSelY1 == ClickStat.iSelY2)
                {
                    ...
                    {
                        Log.ShowMessage("XYT", Chip[ClickStat.iSelC2, ClickStat.iSelR2].dDataX.ToString() + "," + 
                                               Chip[ClickStat.iSelC2, ClickStat.iSelR2].dDataY.ToString() + "," + 
                                               Chip[ClickStat.iSelC2, ClickStat.iSelR2].dDataT.ToString());
