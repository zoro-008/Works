    ...
    public class CMotrMan
    {
        ...
        public bool Init(EN_LAN_SEL _eLanSel,string _sParaFolderPath,int _iMaxMtrCnt,EN_MOTR_SEL [] _eMotrSels ,CDioMan _Dio )
        {
            ...
            for (int i = 0; i < m_iMaxMotr; i++ )
            {
                ...
                if(_eMotrSels[i] == EN_MOTR_SEL.msAXL)
                {
                    Mtr[i] = new MotionAXL.CMotor();
                }
                else if (_eMotrSels[i] == EN_MOTR_SEL.msNMC2)
                {
                    Mtr[i] = new MotionNMC2.CMotor();
                    
                }
                else if (_eMotrSels[i] == EN_MOTR_SEL.msJuni)
                {
                    Mtr[i] = new MotionJuni.CMotor();
                }
                else
                {
        ...
        public bool GetStopInpos(int _iMotrNo)
        {
            ...
            //주니 때문에 추가 2016 10.02 선
            if (!Mtr[_iMotrNo].GetInPosSgnl()) return false;            

            if (Para[_iMotrNo].dInposition == 0.0) return true;
        ...
        public bool CmprPos(int _iMotrNo, double _dPos, double _dRange = 0.0)
        {
            ...
            bool isOk =    (Math.Abs(GetEncPos(_iMotrNo) - _dPos) < dRange || !Para[_iMotrNo].bExistEnc)
                        && (Math.Abs(GetCmdPos(_iMotrNo) - _dPos) < dRange);
        ...
        public bool GoHome(int _iMotrNo)
        {
            ...
            double dMinPos = (360 * Para[_iMotrNo].iPulsePerRev / Para[_iMotrNo].dUnitPerRev) - 1;
            double dMaxPos = (0.0 * Para[_iMotrNo].iPulsePerRev / Para[_iMotrNo].dUnitPerRev) ;

            return Mtr[_iMotrNo].GoHome(dVel1, dVel2, dAcc, dMinPos, dMaxPos);
        }
        ...
        {
            bool bExistAlarm = false;
            for (int i = 0; i < m_iMaxMotr; i++)
            {
                if (Mtr[i].GetAlarmSgnl())
                {
                    Mtr[i].SetReset(true);
                    bExistAlarm = true;
                }
                //if(i != 4 || i != 5){if(Mtr[i].GetAlarmSgnl()) Mtr[i].SetReset(true ); }
                //else                {if(Mtr[i].GetAlarmSgnl()) Mtr[i].SetReset(false); }
                //원래 Reset으로 되어있던거. 되는지 확인해봐야함. 
            }//sleep이 필요할수 있음.
            if(bExistAlarm)Thread.Sleep(100);

            for (int i = 0; i < m_iMaxMotr; i++)
            {
                Mtr[i].SetReset(false);    //원래 Reset으로 되어있던거. 되는지 확인해봐야함. 
                //if (i != 4 || i != 5) {if (Mtr[i].GetAlarmSgnl()) Mtr[i].SetReset(false); }
                //else                  {if (Mtr[i].GetAlarmSgnl()) Mtr[i].SetReset(true);  }
        ...
        public void GoAbsRun(int _iMotrNo, double _dPos)
        {
            GoAbs(_iMotrNo , _dPos , Para[_iMotrNo].dRunSpeed , Para[_iMotrNo].dAcceleration , Para[_iMotrNo].dDeceleration );
        }
                ...
                if (GetServo(i) && GetHomeDone(i) && !GetStop(i))
                {
                    ...
                    if (Para[i].dMaxPos != 0.0)//+리밋 포지션 세팅 한경우만.
                    {
                        if ((dPos > Para[i].dMaxPos + Para[i].dInposition) && Stat[i].bMovingP)
                        {
                                ...
                                Stop(i);
                                Log.Trace("Motr Stop", "+Position Limit Stop");
        ...
        public void CycleRpt(int _iMotrNo)
        {
            ...
            if(GetStop(_iMotrNo))
            {
                ...
                {
                    if (GetCmdPos(_iMotrNo) == Para[_iMotrNo].dFstPos)
                    {
        ...
        public void SetDoublePara(int _iMotrNo , string _sParaName , double _dValue)
        {
            Mtr[_iMotrNo].SetDoublePara(_sParaName , _dValue);
        }

        public int GetIntStat(int _iMotrNo, string _sStatName)
        {
            return Mtr[_iMotrNo].GetIntStat(_sStatName);
        }
    }
