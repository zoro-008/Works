    ...
    public class CDioMan
    {
        ...
        {    
            ...
            for (int i = 0 ; i < m_iMaxOut ; i++) {
                ...
                //미친 로그쉐끼럽 전값이랑 비교해서 로그 남김. dw
                if (!m_aOut[i].Para.bNotLog && m_aOut[i].Stat.bVtrVal != m_aOut[i].Stat.bPreVal)
                {
                    Log.Trace(string.Format("OUTPUT({0:000}) {1} is {2} with Delay {3}", i, m_aOut[i].Para.sEnum, m_aOut[i].Stat.bVtrVal ? "On" : "Off", m_aOut[i].Stat.bVtrVal ? m_aOut[i].Para.iOnDelay : m_aOut[i].Para.iOffDelay),ForContext.Dyi);
                }
                m_aOut[i].Stat.bPreVal =  m_aOut[i].Stat.bVtrVal ;
            ...
            for (int i = 0 ; i < m_iMaxIn ; i++) {
                ...
                if(m_aIn[i].Stat.bVtrVal != m_aIn[i].Stat.bAtrVal) {
                    ...
                    {
                        Log.Trace(string.Format("INPUT({0:000}) {1} is {2} with Delay {3}", i, m_aIn[i].Para.sEnum, m_aIn[i].Stat.bAtrVal ? "On" : "Off", m_aIn[i].Stat.bAtrVal ? m_aIn[i].Para.iOnDelay : m_aIn[i].Para.iOffDelay), ForContext.Dxi);
                    }
                }
                else {
                    m_aIn[i].Stat.bUpEdge = false ;
        ...
        public void SetYTestMode(int _iNo , bool _bVal)
        {
            ...
            Log.Trace(string.Format("OUTPUT({0:000}) {1} is {2} (Direct)", _iNo, m_aOut[_iNo].Para.sEnum, _bVal ? "On" : "Off"), ForContext.Dyi);
        ...
        public bool GetY(int _iNo , bool _bDirect=false)
        {
            ...
            //액추얼로 리턴하면 실제하드웨어 없을때 문제되는듯.
            //IO메니저는 vtr만 사용 하는게 맞는 컨셉인듯함. 선계원.

            //아 그럼 실제 출력이 안나가는데도 켜지는데 단순히 봣을때 출력이 나갈때 켜지는게 맞는듯해서 수정 dw 로그는 남는거는 수정.
            return m_aOut[_iNo].Para.bInv ? !m_aOut[_iNo].Stat.bAtrVal : m_aOut[_iNo].Stat.bAtrVal;
            //return m_aOut[_iNo].Para.bInv ? !m_aOut[_iNo].Stat.bVtrVal : m_aOut[_iNo].Stat.bVtrVal;
        ...
        public void SetAllInputName()
        {
            for (int i = 0; i < m_iMaxIn; i++)
            {
                m_aIn[i].Para.sName = m_aIn[i].Para.sEnum;
            }
        }
        public void SetAllOutputName()
        {
            for (int i = 0; i < m_iMaxOut; i++)
            {
                m_aOut[i].Para.sName = m_aOut[i].Para.sEnum;
            }
        }

        public void SetAllInputAdd()
        {
            for (int i = 0; i < m_iMaxIn; i++)
            {
                m_aIn[i].Para.iAdd = i;
        ...
        public void SetAllOutputAdd()
        {
            for (int i = 0; i < m_iMaxOut; i++)
            {
                m_aOut[i].Para.iAdd = i;
            }
        }
