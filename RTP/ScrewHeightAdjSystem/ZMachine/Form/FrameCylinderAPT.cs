...
namespace Machine
{
    public partial class FrameCylinderAPT : Form
    {
        ...
        private void evFwd_Click(object sender, EventArgs e)
        {
            ...
            if (SEQ.ADJ.CheckSafe(m_iActrId, fb.Fwd , true)) ML.CL_Move(m_iActrId, fb.Fwd);
        }
        ...
        private void evBwd_Click(object sender, EventArgs e)
        {
            ...
            Log.Trace(m_sActrName + "_" + sText + " Button Clicked", ForContext.Frm);
            if (m_iActrId == ci.ADJ_AdjustUpDn)
            {
                MM.SetManCycle(mc.NutCylDown);
            }
            else
            {
                if (SEQ.ADJ.CheckSafe(m_iActrId, fb.Bwd, true)) ML.CL_Move(m_iActrId, fb.Bwd);
            }
