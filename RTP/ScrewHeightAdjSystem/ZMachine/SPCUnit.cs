...
namespace Machine
{
    ...
    public struct TLotData
    {
        ...
        public double TackTime       ;

        public int    WorkCount      ;
        public int    HghtNGCount    ;
        public int    HighTqNGCount  ;
        public int    LowTqNGCount   ;
    ...
    public class CLot : DataHandler<TLotData>
    {
        ...
        public void Update(string _sCrntLotNo, EN_SEQ_STAT Stat, string _sCrntDev, bool _bMaint)
        {
            ...
            if (LotOpened) {
                ...
                SaveDataIni(Data.StartedAt, true);
            }
            else if (LotEnded){ //Adjust사이클중에 체결 측정 사이클에서 1개 생산시에 SaveDataIni을 쓰는 패턴이 있어서 같이 봐야함.
                Data.WorkCount    = OM.EqpStat.iCrntWorkCount  ;
                Data.TotalCount   = OM.EqpStat.iTotalWorkCount ;
                Data.HghtNGCount       = OM.EqpStat.iHghtNG         ;
                Data.HighTqNGCount     = OM.EqpStat.iHighTqNG       ;
                Data.LowTqNGCount      = OM.EqpStat.iLowTqNG        ;
                Data.EndedAt      = DateTime.Now.ToOADate() ;
                SaveDataIni(Data.StartedAt, false);
                //이위로는 기존랏 처리.
            }
            else if (LotChanged)
            {
        ...
        public void DispLotInfo(ListView _lvLotInfo) //오퍼레이션 창용.
        {
            ...
            int iCnt = 0 ;
            //WorkCnt
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "LOT NO";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = LOT.GetLotNo();
            
            //WorkCnt
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "WORK COUNT";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = OM.EqpStat.iCrntWorkCount.ToString() + "/" + OM.EqpStat.iTotalWorkCount.ToString();
            
            ////NG
            //_lvLotInfo.Items[iCnt  ].SubItems[0].Text = "높이NG";
            //_lvLotInfo.Items[iCnt++].SubItems[1].Text = OM.EqpStat.iHghtNG.ToString();
            //
            ////NG
            //_lvLotInfo.Items[iCnt  ].SubItems[0].Text = "상한토크NG";
            //_lvLotInfo.Items[iCnt++].SubItems[1].Text = OM.EqpStat.iHighTqNG.ToString();
            //
            ////NG
            //_lvLotInfo.Items[iCnt  ].SubItems[0].Text = "하한토크NG";
            //_lvLotInfo.Items[iCnt++].SubItems[1].Text = OM.EqpStat.iLowTqNG.ToString();

            //Good
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "GOOD COUNT";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = OM.EqpStat.iGoodCount.ToString();
            
            //Yield
            double dYield = OM.EqpStat.iCrntWorkCount > 0 ? (OM.EqpStat.iCrntWorkCount - OM.EqpStat.iHghtNG) / (double)OM.EqpStat.iCrntWorkCount : 0.0 ;
            dYield = dYield * 100 ;
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "YIELD";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = dYield.ToString("N2");
            
            //SttTime
            tDateTime = DateTime.FromOADate(Data.StartedAt);
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "START TIME";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = tDateTime.ToString("HH:mm:ss");

            //SttTime
            tDateTime = DateTime.FromOADate(OM.EqpStat.dCycleTime);
            _lvLotInfo.Items[iCnt  ].SubItems[0].Text = "TACK TIME";
            _lvLotInfo.Items[iCnt++].SubItems[1].Text = tDateTime.ToString("ss:fff");
            

            ////TickTime
            //string sTemp = string.Format("{0:0.00}",  //dWorkTime);
            //_lvLotInfo.Items[8].SubItems[0].Text = "CYCLE TIME";
    ...
    public class DataHandler<T> where T : struct//StartedAt T구성요소중에  StartedAt  은 꼭 있어야함.
    {
        ...
        public void SaveDataIni(bool _bLotAdded = false)
        {
            SaveDataIni(DateTime.Now.ToOADate(), _bLotAdded);
        }
        public void SaveDataIni(double _dOaSaveDate, bool _bLotAdded = false)
        {
            SaveDataIni(_dOaSaveDate , ref Data, _bLotAdded);
        }
        public void SaveDataIni(double _dOaSaveDate , ref T _tData, bool _bLotAdded = false)
        {
            string sPath;
            DateTime tDateTime;
        ...
        public void DispData(DateTime _tSttData, DateTime _tEndData, ListView _lvTable) //true면 에러넘버 , false면 시간.
        {
            ...
            //컬럼추가 하고 이름을 넣는다.
            //20191118 NG 카운트 SPC에서 보이지 않게 해달라고 해서 일단 이렇게 바꾼다.
            //나중에 SPC 항목에서 다 안나오면 이 부분 수정
    ...
    public static class SPC
    {
        ...
        public static CSpcSubUnit SUB = new CSpcSubUnit();
