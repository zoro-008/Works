...
namespace Machine
{
    ...
    public class CArrayData
    {
        ...
        public struct TData
        {
            ...
            public double dTgtTq0;
            public double dMaxTq0;
            public double dTgtTq1;
            public double dMaxTq1;
            public double dTgtTq2;
            public double dMaxTq2;
            public double dTgtTq3;
            public double dMaxTq3;

            public double dLowTq ; //하한토크 에러 발생했을 때의 체결토크값. 옵션 체결에서 하한토크에러 발생 시 해당 체결 순서에 있는 체결 토크 값 저장

            public string sResultNg;
        ...
        #endregion
        public void LoadData(string _sPath, bool _IsLoad)
        {
            string sSection = "ARRAY_" + m_sName;
            if (_IsLoad)
            {
                Data = new TData();
                Data.sResultNg = "";
                CAutoIniFile.LoadStruct(_sPath, sSection, ref Data);
            }
            else
            {
                CAutoIniFile.SaveStruct(_sPath, sSection, ref Data);
            }
        ...
        public void Trace(string sTag = "", [CallerLineNumber] int sourceLineNumber = 0, [CallerMemberName] string memberName = "", [CallerFilePath] string sourceFilePath = "")
        {
            string sMsg;
            for (int i = 0 ; i < m_iMaxRow ; i++)
            {
        ...
        public void SetStat(cs _eStat)
        {
            ...
            if (_eStat == cs.None)
            {
                ...
                m_iSubStep = 0;
                Data = new TData();
                Data.sResultNg = ""; //개극혐 이번만 쓰고 지울꺼임 어떻게 하는지 몰라서 이렇게 넣음
            }
