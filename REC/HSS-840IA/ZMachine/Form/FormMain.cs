...
namespace Machine
{
    public partial class FormMain : Form
    {
        ...
        private void tmUpdate_Tick(object sender, EventArgs e)
        {
            ...
            //if (SEQ.ScreenRecord.state.bPaused) lbOptn3.Text = "PAUSE";
            //else                                lbOptn3.Text = "MONITOR RECORD";
            //
            //if (SEQ.ScreenRecord.state.bRecording) pnOptn3.Visible = SEQ._bFlick && OM.CmnOptn.bUseRecord;
            //else                                   pnOptn3.Visible = OM.CmnOptn.bUseRecord;
            ...
            try
            {
                switch (m.Msg)
                {
                    ...
                    case WM_DEVICECHANGE:
                        ...
                        {
                                 if (OM.DevInfo.iMacroType == 0) SEQ.XRYD.bDeviceChange = true;
                            else if (OM.DevInfo.iMacroType == 1) SEQ.XRYE.bDeviceChange = true;
                        }
                        return;
                }
            }
        ...
        {
            ML.MT_SetHomeDone(mi.LODR_ZElev, true);
            ML.MT_SetHomeDone(mi.INDX_XRail, true);
            ML.MT_SetHomeDone(mi.XRAY_XFltr, true);
            ML.MT_SetHomeDone(mi.XRAY_ZXRay, true);
            ML.MT_SetHomeDone(mi.XRAY_XCnct, true);
        }

        private void lbOptn3_Click(object sender, EventArgs e)
        {
            //SEQ.ScreenRecord.StopRecording(); 
        }
    }
}
