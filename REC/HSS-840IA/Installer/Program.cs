    ...
    class Program
    {
        ...
        */
        ...
        static private void SetInfo()
        {
            ...
            info.File.Add(@"\Bin\Dll\SML.dll"                    );
            info.File.Add(@"\Bin\Dll\MotionInterface.dll"        );
            info.File.Add(@"\Bin\Dll\MotionAXL.dll"              );

            info.File.Add(@"\Bin\Dll\Microsoft.Expression.Encoder.dll"          );
            info.File.Add(@"\Bin\Dll\Microsoft.Expression.Encoder.Types.dll"    );
            info.File.Add(@"\Bin\Dll\Microsoft.Expression.Encoder.Utilities.dll");
            //info.File.Add(@"\Bin\Dll\ko\SML.resources.dll"     );
            //info.File.Add(@"\Bin\Dll\zh-Hans\SML.resources.dll");

            info.File.Add(@"\Bin\Dll\Common.dll"               );

            FileInfo file;
            for(int i=0; i<info.File.Count; i++)
            {
                //info.SetOutPath.Add(@"""" + Path.GetDirectoryName(info.File[i]) + @"""");
