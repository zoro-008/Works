...
//      [CategoryAttribute("Motor Para" ), DisplayNameAttribute("Physical No"             )]public int        iPhysicalNo   {get; set;} //실제모터 물리 어드레스.
    ...
    public class CMotrMan
    {
        ...
        public bool GoHome(int _iMotrNo)
        {
            ...
            Stat[_iMotrNo].bHomeDone = false ;
            Stat[_iMotrNo].bHoming = true  ;
        ...
        public bool GetHomeDone(int _iMotrNo)
        {
            ...
            return Stat[_iMotrNo].bHomeDone ;//Mtr[_iMotrNo].GetHomeDone(i)
        }
        ...
        public bool GetHomeDoneAll()
        {
            for (int i = 0; i < m_iMaxMotr; i++)
            {
                if(!Stat[i].bHomeDone) return false;
            }
        ...
        protected void Go(int _iMotrNo, double _dPos, double _dVel, double _dAcc, double _dDec)
        {
            ...
            if (_dPos > Para[_iMotrNo].dMaxPos)
            {
                ...
                Log.ShowMessage("ERR Motr"+_iMotrNo , "Max Position Over" + _dPos);
            ...
            {
                Log.ShowMessage("ERR Motr"+_iMotrNo , "Min Position Over" + _dPos);
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < m_iMaxMotr; i++)
            {
                ...
                //조그 구동시에 Target Pos 설정.
                if(!Mtr[i].GetStop()) {
                    if(Stat[i].bJogN || Stat[i].bJogP) {
                        Stat[i].dTrgPos = GetCmdPos(i);
                    }
                } 
                else {
                    Stat[i].bJogN = false ;
                ...
                if(Stat[i].bHoming){
                    Stat[i].bHomeDone=Mtr[i].GetHomeDone();
                    Stat[i].dTrgPos = GetCmdPos(i);
                }
                Stat[i].bHoming = GetHoming(i);
                ...
                if (!GetStop(i) && GetServo(i) && !Stat[i].bHoming)
                {
                    //+리밋 센서 처리.
