...
{    
    ...
    public class CMotor:IMotor
    {
        ...
        public bool GetHoming()
        {
            ...
            if (iRet != iPreHomeRslt)
            {
                switch (iRet)
                {
                    ...
                    case GANTRY_RANGEOVER : MessageBox.Show(new WindowWrapper(handle),"Gantry Offset Rangeover"    , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case STOPED           : MessageBox.Show(new WindowWrapper(handle),"Home Stoped"                , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case SPEED_WRONG      : MessageBox.Show(new WindowWrapper(handle),"Check Home Speed Setting"   , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case SERVO_ALRAM      : MessageBox.Show(new WindowWrapper(handle),"Servo Alram"                , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case POS_DIRCT_NEG_LMT: MessageBox.Show(new WindowWrapper(handle),"Checked -Lmt while +Homing" , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case NEG_DIRCT_POS_LMT: MessageBox.Show(new WindowWrapper(handle),"Checked +Lmt while -Homing" , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
                    case NOT_FOUND        : MessageBox.Show(new WindowWrapper(handle),"HomeSensor desn't found"    , "PhyNo:" + Para.iPhysicalNo.ToString() + "Homing Err"); break;
        ...
        public void SetTrgPos(double[] _dPos,double _dTrgTime,bool _bActual,bool _bOnLevel)
        {
            ...
            //배열 인자에 관해서 모르겠다. ;;
            //나중에 찾아보자.
            CAXM.AxmTriggerOnlyAbs(Para.iPhysicalNo, _dPos.Length, ref _dPos[0]);
