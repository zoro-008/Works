...
namespace Machine
{
    public partial class FormDevice : Form
    {
        ...
        private void btSetting_Click(object sender, EventArgs e)    //DeviceSet Form 띄움
        {
            ...
            FrmMain.FrmDeviceSet.UpdatePattern(true);
        }
...
//            bool bAllArayNone = DM.ARAY[(int)ri.SLD].CheckAllStat(cs.None);
            ...
            PM.Load(sName);  
            //웨이퍼세타 워크를 기준포지션으로 세팅 해놓음.
            PM.SetValue(mi.WSTG_TTble , pv.WSTG_TTbleWfrWork , PM.GetValue(mi.WSTG_TTble , pv.WSTG_TTbleWork));
            PM.Save(sName);  
            SEQ.DispPtrn.Load(sName);
            SEQ.HghtPtrn.Load(sName);                                        
            SEQ.BltPtrn .Load(sName);                      pbStatus.Value    = 70;
            pbStatus.Value = 100;
            ...
            OM.EqpStat.bNeedCheckPckr = true ;
            ...
            TimeOut.Clear();
            while(!SEQ.Dispr.SetLoadCh(OM.DevOptn.iDspCh + 1)){
                Thread.Sleep(1);
                if(TimeOut.OnDelay(2000)) { 
                    SM.ER_SetErr(ei.ETC_RS232,"디스펜서 채널 변경 통신 타임아웃");
                    break;
                }
            }
