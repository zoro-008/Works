...
namespace Machine
{
    ...
    public partial class FraMotr : Form
    {
        ...
        private void tmUpdate_Tick(object sender, EventArgs e)
        {
            ...
            LbStat1.BackColor = SML.MT.GetNLimSnsr  (m_iId) ? Color.Lime : Color.Silver;
            LbStat2.BackColor = SML.MT.GetHomeSnsr  (m_iId) ? Color.Lime : Color.Silver;
            LbStat3.BackColor = SML.MT.GetPLimSnsr  (m_iId) ? Color.Lime : Color.Silver;
            LbStat4.BackColor = SML.MT.GetAlarmSgnl (m_iId) ? Color.Lime : Color.Silver;
            LbStat5.BackColor = SML.MT.GetServo     (m_iId) ? Color.Lime : Color.Silver;
            LbStat6.BackColor = SML.MT.GetStop      (m_iId) ? Color.Lime : Color.Silver;
            LbStat7.BackColor = SML.MT.GetHomeDone  (m_iId) ? Color.Lime : Color.Silver;

            LbCmdPos.Text = SML.MT.GetCmdPos(m_iId).ToString();
            LbEncPos.Text = SML.MT.GetEncPos(m_iId).ToString();
        ...
        {
            if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog) SML.MT.Stop(m_iId);
        }
        ...
        private void btNeg_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (!cbSlow.Checked)
            {
                     if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog  ) SML.MT.JogN    (m_iId);
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utMove ) SML.MT.GoIncMan(m_iId, -(Para.dUnit));
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utPitch) SML.MT.GoIncMan(m_iId, -(Para.dUnit));
                else return;
            }
            else
            {
                     if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog  ) SML.MT.JogN     (m_iId);
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utMove ) SML.MT.GoIncSlow(m_iId, -(Para.dUnit));
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utPitch) SML.MT.GoIncSlow(m_iId, -(Para.dUnit));
                else return;
            }
        }
        ...
        private void btPos_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (!cbSlow.Checked)
            {
                     if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog  ) SML.MT.JogP    (m_iId);
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utMove ) SML.MT.GoIncMan(m_iId, Para.dUnit);
                else if (Para.iUnitType == (int)EN_UNIT_TYPE.utPitch) SML.MT.GoIncMan(m_iId, Para.dUnit);
                else return;
            }
