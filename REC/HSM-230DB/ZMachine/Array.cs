...
namespace Machine
{
    ...
    {
        public cs Stat;
        public string Data; //Ver. 1.0.5.0 2018.08.09 바코드 데이터 남기기위해 추가
        public void Clear()
        {
            Stat = cs.None;
            Data = "";
            ...
            if(IsLoad) {                                
                ...
                try
                {
                    ...
                    for (int r = 0; r < m_iMaxRow; r++)
                    {
                        for (int c = 0; c < m_iMaxCol; c++)
                        {
                            //sIndex = string.Format("Col{0:000}_Row{0:000}_Data", c, r);
                            sIndex = string.Format("Col{0:000}", c);
                            sIndex += string.Format("_Row{0:000}_Data", r);
                            Config.GetValue(sArayName, sIndex, out sTemp);
                            m_tChips[c, r].Data = sTemp;
                        }
                    }
                }
                catch(Exception e)
                ...
                for (int r = 0; r < m_iMaxRow; r++)
                {
                    for (int c = 0; c < m_iMaxCol; c++)
                    {
                        sIndex  = string.Format("Col{0:000}", c);
                        sIndex += string.Format("_Row{0:000}_Data", r);
                        sTemp = m_tChips[c, r].Data;
                        if (m_tChips[c, r].Data == null) sTemp = "";
                        Config.SetValue(sArayName, sIndex, sTemp);
                    }
                }
