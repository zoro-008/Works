...
namespace Machine
{
    ...
    public class MotrPstn
    {
        ...
        public int iNameWidth = 305;
        public int iValueWidth = 150;
        public int iGoWidth = 50;
        public int iInputWidth = 70;
        public int iMinMaxWidthName = 50; //Name
        public int iMinMaxWidthVal  = 80; //Value 775
        public int iSpeedPerName    = 80;
        public int iSpeedPerVal     = 40;
        ...
        private void btGoClick(object sender, EventArgs e)
        {
            ...
            if(!ML.MT_GetStop((mi)m_uMotrId)) ML.MT_Stop((mi)m_uMotrId);
            ...
            if (!bRet) return;
            ML.MT_GoAbsRun((mi)m_uMotrId, PM.GetValue(m_uMotrId, uPstnNo), PM.GetValueSpdPer(m_uMotrId, uPstnNo) );
        ...
        private void btInputClick(object sender, EventArgs e)
        {
            ...
            bool   bServo = ML.MT_GetServo((mi)m_uMotrId);
            ...
            if (bServo)
            {
                dPos      = ML.MT_GetCmdPos((mi)m_uMotrId);
                sPos      = dPos.ToString("N4");
                dInputPos = double.Parse(sPos);
            }
            else
            {
                dPos      = ML.MT_GetEncPos((mi)m_uMotrId);
                sPos      = dPos.ToString("N4");
                dInputPos = double.Parse(sPos);
            ...
            Log.Trace("Form DeviceSet Input Button Clicked ("  + ML.MT_GetName((mi)m_uMotrId) + " " + GetName(uPstnNo).Trim() + " " +
                       GetValue(uPstnNo) + " -> " + sPos + ")", ti.Frm);
        ...
        public void SetValue(uint _uPstnNo, double _dValue)
        {
            ...
            PstnValue[_uPstnNo].dValue = dValue;
            ...
            if (lvPstnView.Parent != null)
            {
                if (lvPstnView.InvokeRequired)
                {
                    lvPstnView.Invoke(new MethodInvoker(delegate() { lvPstnView.Items[(int)_uPstnNo].SubItems[1].Text = PstnValue[_uPstnNo].dValue.ToString(); }));
                }
                else
                {
        ...
        public void SetValueMin(uint _uPstnNo, double _dValue)
        {
            PstnValue[_uPstnNo].dMin = _dValue;
        ...
        public void SetValueMax(uint _uPstnNo, double _dValue)
        {
            PstnValue[_uPstnNo].dMax = _dValue;
        ...
        public void SetValueSpd(uint _uPstnNo, int _iValue)
        {
            int iValue = _iValue;
            if (iValue <=  0) iValue = 100;
            if (iValue > 100) iValue = 100;
            PstnValue[_uPstnNo].iSpdPer = iValue;
    ...
    public static class PM
    {
        ...
        public static uint[] PstnCnt = { (uint)pv.MAX_PSTN_MOTR0 ,
        ...
        public static double GetValueMin(uint _uAxisNo, uint _uPstnNo)
        {
            return MotrPstn[_uAxisNo].GetValueMin(_uPstnNo);
        }
        public static double GetValueMax(uint _uAxisNo, uint _uPstnNo)
        {
            return MotrPstn[_uAxisNo].GetValueMax(_uPstnNo);
        }
        ...
        public static double GetValueMin(mi _eAxisNo, pv _ePstnNo)
        {
            return GetValueMin((uint)_eAxisNo, (uint)_ePstnNo);
        }

        public static double GetValueMax(mi _eAxisNo, pv _ePstnNo)
        {
            return GetValueMax((uint)_eAxisNo, (uint)_ePstnNo);
        }
        ...
        public static void Load(string _sJobName)
        {
            ...
            for (int m = 0; m < m_iMaxMotr; m++)
            {
                ...
                for (int i = 0; i < MotrPstn[m].PstnValue.Length; i++)
                {
                    ...
                    if (MotrPstn[m].PstnValue[i].dMax <= 0) MotrPstn[m].PstnValue[i].dMax = ML.MT_GetMaxPosition((mi)m);
        ...
        public static void UpdatePstn(bool _bToTable)
        {
            if (_bToTable)
            {
                for (uint m = 0; m < (uint)mi.MAX_MOTR; m++)
                {
                    ...
                    {
                        MotrPstn[m].lvPstnView.Items[i].SubItems[0].Text = MotrPstn[m].PstnValue[(int)i].sName;
