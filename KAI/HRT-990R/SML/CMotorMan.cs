...
//      [CategoryAttribute("Motor Para" ), DisplayNameAttribute("Physical No"             )]public int        iPhysicalNo   {get; set;} //실제모터 물리 어드레스.
    ...
    public class CMotrMan
    {
        ...
        protected void GoRel(int _iMotrNo, double _dPos, double _dVel, double _dAcc, double _dDec)
        {
            if (_iMotrNo < 0 || _iMotrNo >= m_iMaxMotr)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}", _iMotrNo, m_iMaxMotr));
            ...
            double dAbs = GetCmdPos(_iMotrNo) + _dPos ;
            ...
            if (GetStop(_iMotrNo)) { 
                Log.Trace(GetName(_iMotrNo) , "Go From " + GetCmdPos(_iMotrNo).ToString() + " To " + dAbs.ToString() );
            }

            if (dAbs > Para[_iMotrNo].dMaxPos)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is Max Position Over{1}", _iMotrNo, dAbs));
                return;
            }
            if (dAbs < Para[_iMotrNo].dMinPos)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is Min Position Over{1}", _iMotrNo, dAbs));
                return;
            }

            Mtr[_iMotrNo].GoRel(dPos , dVel , dAcc , dDec );
            
            Stat[_iMotrNo].dTrgPos = dAbs;
        }
        ...
        public void GoInc(int _iMotrNo, double _dPos, double _dVel, double _dAcc, double _dDec)
        {
            ...
            GoRel(_iMotrNo, _dPos, _dVel, _dAcc, _dDec);
        }
        ...
        public void GoIncVel(int _iMotrNo, double _dPos, double _dVel)
        {
            ...
            GoRel(_iMotrNo, _dPos, _dVel, Para[_iMotrNo].dAcceleration, Para[_iMotrNo].dDeceleration);
        }
        ...
        public void GoIncRun(int _iMotrNo, double _dPos)
        {
            ...
            GoRel(_iMotrNo, _dPos, Para[_iMotrNo].dRunSpeed, Para[_iMotrNo].dAcceleration, Para[_iMotrNo].dDeceleration);
        }
        ...
        public void GoIncMan(int _iMotrNo, double _dPos)
        {
            ...
            GoRel(_iMotrNo, _dPos, Para[_iMotrNo].dManSpeed, Para[_iMotrNo].dAcceleration, Para[_iMotrNo].dDeceleration);
        }
        ...
        public void GoIncSlow(int _iMotrNo, double _dPos)
        {
            ...
            GoRel(_iMotrNo, _dPos, Para[_iMotrNo].dSlowSpeed, Para[_iMotrNo].dAcceleration, Para[_iMotrNo].dDeceleration);
        }
        ...
        public void SetTrgAbs(int _iMotrNo, bool _bMethod, double _dPos, double _dTrgTime, bool _bActual, bool _bOnLevel)
        {
            if (_iMotrNo < 0 || _iMotrNo >= m_iMaxMotr)
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}", _iMotrNo, m_iMaxMotr));
            ...
            double dPosPulses = UnitToPulse(_iMotrNo, _dPos);
            Mtr[_iMotrNo].SetTrgAbs(dPosPulses, _bMethod, _dTrgTime, _bActual, _bOnLevel);
        }
