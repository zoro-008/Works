...
{    
    ...
    public class CMotor:IMotor
    {
        ...
        public bool GoHome(double _dHomeVelFirst, double _dHomeVelLast, double _dHomeAccFirst)
        {
            ...
            //전원 재투입 및 이머전시시에 설정값을 다시 보내 줘야 함... 이것은 모터가 통신 타입이여서 모터에서 설정값을 가지고 있어서 그럼.
            ApplyPara(m_dPulsePerUnit);//야매로 그냥 m_dPulsePerUnit를 보낸다. ApplyPara함수 자체가 클래스 내부에서 쓰는 함수가 아니라 그럼.
