...
namespace Machine
{
    public class ML
    {
        ...
        static public bool         IO_GetX    (xi   _eX, bool _bDirect = false)                        {
            ...
            {
                if(_eX == xi.PickerVacuum) return SM.DIO.GetY((int)yi.PickerVacuum);
                if(_eX == xi.StageVacuum ) return SM.DIO.GetY((int)yi.StageVacuum );
                if(_eX == xi.WaferDtSsr  ) return !DM.ARAY[ri.STG].CheckAllStat(cs.None);
            }
            return SM.DIO.GetX    ((int)_eX, _bDirect);        
        }
