...
namespace Machine
{
    ...
    public delegate bool dgJogCheckSafe(mi _eMotr, EN_JOG_DIRECTION _eDir, EN_UNIT_TYPE _eType, double _dDist);

    public partial class FraMotr : Form
    {
        dgJogCheckSafe JogCheckSafe;
        ...
        public void SetJogCheckSafe(dgJogCheckSafe _dgJogCheckSafe)
        {
            JogCheckSafe = _dgJogCheckSafe;
        ...
        bool bJogDisable = false;
        public void JogDisable(bool _bNotUse = true)
        {
            if (_bNotUse) bJogDisable = true;
        }
        ...
        private void btNeg_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (JogCheckSafe != null)
            {
                bRet = JogCheckSafe((mi)m_eId, EN_JOG_DIRECTION.Neg, (EN_UNIT_TYPE)Para.iUnitType, Para.dUnit);
            }

            //TODO :: 맨날 여기다 치는것좀 빼줘
            //bRet = SEQ.LDR.CheckSafe((mi)m_eId, (pv)(0));

            if (!bRet) return;

            //ML.MT_Stop(m_eId);//여기서 스탑을 시키면 파익스 모터는 스탑과 조그를 거의 동시에 날려서 동작 안하는듯...
            if (!ML.MT_GetStop(m_eId)) return; //이것으로 대체.
            ...
            {
                Log.TraceListView("FrameMotr Form_" + sTemp + " " + sText + " Button Clicked",ForContext.Frm);

                if (!cbSlow.Checked)
                {
                         if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog  && !bJogDisable) ML.MT_JogN    (m_eId);
        ...
        private void btPos_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (JogCheckSafe != null)
            {
                bRet = JogCheckSafe((mi)m_eId, EN_JOG_DIRECTION.Pos, (EN_UNIT_TYPE)Para.iUnitType, Para.dUnit);
            }
            ...
            else
            {
                ...
                if (!cbSlow.Checked)
                {
                         if (Para.iUnitType == (int)EN_UNIT_TYPE.utJog  && !bJogDisable) ML.MT_JogP    (m_eId);
                    else if (Para.iUnitType == (int)EN_UNIT_TYPE.utMove ) ML.MT_GoIncMan(m_eId, Para.dUnit);
                    else if (Para.iUnitType == (int)EN_UNIT_TYPE.utPitch) ML.MT_GoIncMan(m_eId, Para.dUnit);
                    else return;
                }
        ...
        {
            string sText = ((Button)sender).Text;
            Log.TraceListView("FrameMotr Form_" + m_eId.ToString() + " Stop Button Clicked", ForContext.Frm);
            ML.MT_Stop(m_eId);
        }
