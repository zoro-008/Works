...
namespace SMDll2
{
    public class CDioMan
    {
        ...
        EN_LAN_SEL m_eLangSel = EN_LAN_SEL.lsKorean; //Languge Selection.
        ...
        public bool Init(EN_LAN_SEL _eLanSel,string _sParaFolderPath,int _iMaxIn,int _iMaxOut,EN_DIO_SEL _eDioSel)
        {
            ...
            if(_eDioSel == EN_DIO_SEL.dsAXL)
            {
                Dio = new MotionAXL.CDio();   
            }
            else if(_eDioSel == EN_DIO_SEL.dsNMC2)
            {
        ...
        public void SetYTestMode(int _iNo , bool _bVal)
        {
            ...
            Log.Trace("IO Direct Change", string.Format("OUT{0:000}_{1} is {2}", _iNo, m_aOut[_iNo].Para.sEnum, _bVal ? "On" : "Off"));
