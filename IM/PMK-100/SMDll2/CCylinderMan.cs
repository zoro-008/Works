...
namespace SMDll2
{
    public class CCylinder
    {
        ...
        public struct TStat 
        {
            public EN_CYLINDER_POS  eActPos ; //true == Fwd ;
            public EN_CYLINDER_POS  eCmdPos ;
            public bool             bErr    ;
        };
        ...
        public void Reset()
        {
            ...
            if(bVtrlFwdX) Stat.eCmdPos = EN_CYLINDER_POS.cpFwd ;
            if(bVtrlBwdX) Stat.eCmdPos = EN_CYLINDER_POS.cpBwd ;
        ...
        public EN_CYLINDER_POS GetCmd()
        {
            return Stat.eCmdPos ;
        }

        public EN_CYLINDER_POS GetAct()
        {
            return Stat.eActPos ;
        }
        ...
        public bool Move(EN_CYLINDER_POS _bCmd)
        {
            ...
            Log.Trace(Para.sEnum + " Cyl Move",_bCmd == EN_CYLINDER_POS.cpFwd ? "Fwd" : "Bwd");
            
            if( _bCmd == EN_CYLINDER_POS.cpFwd) {
        ...
        public void Update()
        {
            ...
            if(bDelayedFwd) Stat.eActPos = EN_CYLINDER_POS.cpFwd;
            if(bDelayedBwd) Stat.eActPos = EN_CYLINDER_POS.cpBwd;

            if ( bVtrlFwdX && !bVtrlBwdX) Stat.eCmdPos = EN_CYLINDER_POS.cpFwd;
            if (!bVtrlFwdX &&  bVtrlBwdX) Stat.eCmdPos = EN_CYLINDER_POS.cpBwd;

            //OnDelay Timer.
            TimeOutFwd.OnDelay(Stat.eCmdPos == EN_CYLINDER_POS.cpFwd && Stat.eActPos != EN_CYLINDER_POS.cpFwd,Para.iFwdTimeOut);
            TimeOutBwd.OnDelay(Stat.eCmdPos == EN_CYLINDER_POS.cpBwd && Stat.eActPos != EN_CYLINDER_POS.cpBwd,Para.iBwdTimeOut);

            Stat.bErr = TimeOutFwd.OnDelay(Para.iFwdTimeOut) || TimeOutFwd.OnDelay(Para.iBwdTimeOut);
    ...
    public class CCylinderMan//========================================================================================================================
    {
        ...
        public bool Complete(int _iCylNo, EN_CYLINDER_POS _bCmd)
        {
            if (_iCylNo < 0 || _iCylNo >= m_iMaxCylinder) Log.ShowMessageFunc(string.Format("CYL : {0} is not in between 0 and MaxCylinder:{1}", _iCylNo, m_iMaxCylinder));
        ...
        public EN_CYLINDER_POS GetCmd(int _iCylNo)
        {
            if (_iCylNo < 0 || _iCylNo >= m_iMaxCylinder) Log.ShowMessageFunc(string.Format("CYL : {0} is not in between 0 and MaxCylinder:{1}", _iCylNo, m_iMaxCylinder));
            return m_aCylinder[_iCylNo].GetCmd();
        }

        public EN_CYLINDER_POS GetAct(int _iCylNo)
        {
            if (_iCylNo < 0 || _iCylNo >= m_iMaxCylinder) Log.ShowMessageFunc(string.Format("CYL : {0} is not in between 0 and MaxCylinder:{1}", _iCylNo, m_iMaxCylinder));
        ...
        public void GoRpt(int _iDelay , int _iActr1No , int _iActr2No = -1)
        {
            ...
            if (GetCmd(_iActr1No) == EN_CYLINDER_POS.cpFwd && GetCmd(_iActr2No) == EN_CYLINDER_POS.cpFwd) Repeat.bRepeatFwd = true;
            if (GetCmd(_iActr1No) == EN_CYLINDER_POS.cpBwd && GetCmd(_iActr2No) == EN_CYLINDER_POS.cpBwd) Repeat.bRepeatFwd = false;
        ...
        public void RptActr ()
        {
            ...
            { //실린더 1개 맞출때
                //if (Repeat.DelayRepeat.OnDelay(GetCmd(Repeat.iFrstActr) == EN_CYLINDER_POS.cpFwd, Repeat.iDelay))
                if (Repeat.DelayRepeat.OnDelay(true, Repeat.iDelay) && Repeat.bRepeatFwd)
                {
                    Move(Repeat.iFrstActr, EN_CYLINDER_POS.cpBwd);
                    Repeat.DelayRepeat.Clear();
                    Repeat.bRepeatFwd = false;
                }
                //if (Repeat.DelayRepeat.OnDelay(GetCmd(Repeat.iFrstActr) == EN_CYLINDER_POS.cpBwd, Repeat.iDelay))
                if (Repeat.DelayRepeat.OnDelay(true, Repeat.iDelay) && !Repeat.bRepeatFwd)
                {
                    Move(Repeat.iFrstActr, EN_CYLINDER_POS.cpFwd);
                    Repeat.DelayRepeat.Clear();
            ...
            {                      //실린더 2개 같이 맞출때
                ...
                {
                    Move(Repeat.iFrstActr, EN_CYLINDER_POS.cpBwd);
                    Move(Repeat.iScndActr, EN_CYLINDER_POS.cpBwd);
                    Repeat.DelayRepeat.Clear();
                    Repeat.bRepeatFwd = false;
                }
                //if (Repeat.DelayRepeat.OnDelay(GetCmd(Repeat.iFrstActr) == EN_CYLINDER_POS.cpBwd, Repeat.iDelay))
                ...
                {
                    Move(Repeat.iFrstActr, EN_CYLINDER_POS.cpFwd);
                    Move(Repeat.iScndActr, EN_CYLINDER_POS.cpFwd);
                    Repeat.DelayRepeat.Clear();
                    Repeat.bRepeatFwd = true;
                }
            }
        ...
        public void DisplayStatus(int _iActrNo , Label _lbFwdStat , Label _lbBwdStat , Label _lbAlarm)
        {
            ...
            //Display.
            if (m_aCylinder[_iActrNo].Complete(EN_CYLINDER_POS.cpFwd)) _lbFwdStat .BackColor = Color.Lime;
            else                                                       _lbFwdStat .BackColor = Color.Gray;
            if (m_aCylinder[_iActrNo].Complete(EN_CYLINDER_POS.cpBwd)) _lbBwdStat .BackColor = Color.Lime;
