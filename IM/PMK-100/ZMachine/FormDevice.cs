...
namespace Machine
{
    public partial class FormDevice : Form
    {
        ...
        {
            this.Hide();
            EmbededExe.SetCamParent(FrmMain.FrmDeviceSet.GetCamPnHandle());
...
//            bool bAllArayNone = DM.ARAY[(int)ri.SLD].CheckAllStat(cs.None);
            ...
            if (LOT.GetLotOpen())
            {
                Log.ShowMessage("Error", "자제나 메거진이 남아 있으면 잡파일을 바꿀수 없습니다.");
                return;
            }
            ...
            //Vision Job Change.
            VC.SendVisnMsg(VC.sm.Change, OM.DevInfo.sVsnIndx);
...
//            DM.ARAY[(int)ri.SLD].SetStat(cs.None);
        ...
        private void PrintSendMsg()
        {
            ...
            iRepCnt = OM.DevInfo.iMrkReptDelay;
            sRepCnt = iRepCnt.ToString("D4");
