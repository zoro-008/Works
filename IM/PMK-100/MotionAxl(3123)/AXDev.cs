...
//========== 보드 및 모듈 확인함수(Info) - Infomation =================================================================================
    ...
    [DllImport("AXL.dll")] public static extern uint AxmSetScriptCaptionIp(int nAxisNo, long sc, uint uEvent, uint data);
    // 지정 축에 스크립트를 반환한다. - IP
    [DllImport("AXL.dll")] public static extern uint AxmGetScriptCaptionIp(int nAxisNo, long sc, ref uint uEvent, ref uint data);
    ...
    // cmd   : 어떤 내용을 바꿀것인지 선택 SCRCMD를 정의한다.
    // data  : 어떤 Data를 바꿀것인지 선택
    [DllImport("AXL.dll")] public static extern uint AxmSetScriptCaptionQi(int nAxisNo, long sc, uint uEvent, uint cmd, uint data);
    // 지정 축에 스크립트를 반환한다. - QI
    [DllImport("AXL.dll")] public static extern uint AxmGetScriptCaptionQi(int nAxisNo, long sc, ref uint uEvent, ref uint cmd, ref uint data);
