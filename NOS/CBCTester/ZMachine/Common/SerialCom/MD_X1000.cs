    ...
    {   
        ...
        public bool CycleTrigger(bool _bInit = false )
        {
            ...
            {            
                ...
                case 10:
                    m_CycleTime.Start();//에러 나중에 에러 메세지 방식으로 바꿔서 밖에서 에러 띄우자.
                    if(!IO_GetX(xReady  )) { sErrMsg = "Ready Signal is not On"; /*ER_SetErr(ei.LSC_ComErr , sErrMsg);*/ return true ; }
                    if( IO_GetX(xWorking)) { sErrMsg = "Working Signal is On";   /*ER_SetErr(ei.LSC_ComErr , sErrMsg);*/ return true ; }
                    if( IO_GetX(xWarning)) { sErrMsg = "Warning Signal is On";   /*ER_SetErr(ei.LSC_ComErr , sErrMsg);*/ return true ; }
                    if( IO_GetX(xError  )) { sErrMsg = "Error Signal is On";     /*ER_SetErr(ei.LSC_ComErr , sErrMsg);*/ return true ; }
                    if( IO_GetY(yTrigger)) { sErrMsg = "Trigger Signal is On";   /*ER_SetErr(ei.LSC_ComErr , sErrMsg);*/ return true ; }
