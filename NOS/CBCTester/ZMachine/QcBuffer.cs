...
namespace Machine
{
    public class QcBuffer : Part
    {
        ...
        protected CDelayTimer m_tmUnFreeze  ;      
        ...
        public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                bool isUnFreeze  =  DM.ARAY[ri.BFR].IsExist     (cs.Freeze ) && m_tmUnFreeze.OnDelay(OM.CmnOptn.iUnFreezeMin * 60);                
                bool isLtPickBfr =  DM.ARAY[ri.BFR].IsExist     (cs.Shake  ) && DM.ARAY[ri.LPK].CheckAllStat(cs.None)      && DM.ARAY[ri.SUT].CheckAllStat(cs.None);
                ...
                bool isLtPlceBfr =  DM.ARAY[ri.LPK].CheckAllStat(cs.WorkEnd) && DM.ARAY[ri.BFR].CheckAllStat(cs.None)      ;
                bool isRtPickBfr =  DM.ARAY[ri.RPK].CheckAllStat(cs.None   ) && DM.ARAY[ri.BFR].CheckAllStat(cs.WorkEnd)   ;
                bool isRtPlceFrg =  DM.ARAY[ri.RPK].CheckAllStat(cs.WorkEnd) && DM.ARAY[ri.FRG].CheckAllStat(cs.None)      ;
                ...
                //Normal Decide Step.
                     if (isRtPlceFrg) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.RtPlceFrg ; }
                else if (isRtPickBfr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.RtPickBfr ; }
                else if (isLtPlceBfr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.LtPlceBfr ; }
                else if (isLtPickSut) { DM.ARAY[ri.SUT].Trace(m_iPartId); Step.eSeq = sc.LtPickSut ; }
                else if (isLtPlceSut) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.LtPlceSut ; }
                else if (isLtPickBfr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.LtPickBfr ; }
                else if (isUnFreeze ) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.UnFreeze  ; }
                else if (isRtPlceBfr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.RtPlceBfr ; }
                else if (isRtPickFrg) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.RtPickFrg ; }
