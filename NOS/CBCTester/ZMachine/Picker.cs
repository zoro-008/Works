...
namespace Machine
{
    public class Picker : Part
    {
        ...
        {
            if(OM.EqpStat.bAutoQCMode) return AutorunNm();
            else                       return AutorunQC();
        }
        //인터페이스 상속 끝.==================================================
        #endregion        
        ...
        public bool AutorunNm() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //사이클
                bool isPickLdr =  DM.ARAY[ri.LDR].CheckAllStat(cs.Shake  ) && DM.ARAY[ri.PKR].CheckAllStat(cs.None);
                ...
                //Normal Decide Step.
                     if (isPlceLdr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PlceLdr ; }
                else if (isPickSut) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PickSut ; }
                else if (isBarcode) { DM.ARAY[ri.SUT].Trace(m_iPartId); Step.eSeq = sc.Barcode ; }
                else if (isPlceSut) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PlceSut ; }
                else if (isShake  ) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.Shake   ; }
                else if (isPickLdr) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PickLdr ; }
        ...
        public bool AutorunQC() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //Normal Decide Step.
                     ...
                     if (isShake  ) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.Shake   ; }
                else if (isPlceSut) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PlceSut ; }
                else if (isBarcode) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.Barcode ; }
                else if (isPickSut) { DM.ARAY[ri.PKR].Trace(m_iPartId); Step.eSeq = sc.PickSut ; }
                else if (isWorkEnd) { Stat.bWorkEnd = true; return true; }             
                Stat.bWorkEnd = false;
        ...
        public bool CyclePickLdr()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 12:
                    if(!MT_GetStopPos(mi.PKR_ZPckr , pv.PKR_ZPckrShake))return false ;
                    MoveCyl(ci.PKR_PkrFtRr      , fb.Fwd);
                    MoveCyl(ci.PKR_PkrShakeUpDn , fb.Bwd);
                    Step.iCycle++;
                    return false ;

                case 13:
                    if(!CL_Complete(ci.PKR_PkrFtRr      , fb.Fwd))return false ;
                    if(!CL_Complete(ci.PKR_PkrShakeUpDn , fb.Bwd))return false ;
                    MoveMotr(mi.PKR_ZPckr , pv.PKR_ZPckrRack );
                    Step.iCycle++;
                    return false ;

                case 14:
                    if(!MT_GetStopPos(mi.PKR_ZPckr , pv.PKR_ZPckrRack))return false ;
                ...
                case 17:
                    ...
                    MoveCyl(ci.PKR_PkrFtRr , fb.Bwd);
                    Step.iCycle++;
                    return false ;

                case 18:
                    if(!CL_Complete(ci.PKR_PkrFtRr , fb.Bwd))return false ;
        ...
        public bool CycleShake()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 14:
                    if(!CL_Complete(ci.PKR_PkrShakeUpDn , fb.Bwd))return false ;
                    iCrntShakeCnt++;
                    if(iCrntShakeCnt < OM.CmnOptn.iShakeCnt)
                    {
                        Step.iCycle = 12 ;
                        return false ;
                    }
        ...
        public bool CyclePlceSut()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 13:
                    ...
                    MoveMotr(mi.PKR_ZPckr , pv.PKR_ZPckrSuttle , -0.2); //플레이스는 0.2씩 오프셑을 둔다.
                    Step.iCycle++;
                    return false ;

                case 14:
                    if(!MT_GetStopPos(mi.PKR_YSutl , pv.PKR_YSutlPicker , -0.2))return false ;
                    MoveCyl(ci.PKR_PkrClampClOp , fb.Bwd);
                    Step.iCycle++;
                    return false ;
        ...
        public bool CyclePickSut()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 13:
                    ...
                    if(!CL_Complete(ci.PKR_PkrClampClOp , fb.Bwd))return false; 
                    if(!MT_GetStopPos(mi.PKR_YSutl , pv.PKR_YSutlPicker))return false ;
                    MoveMotr(mi.PKR_ZPckr , pv.PKR_ZPckrSuttle ); //플레이스는 0.2씩 오프셑을 둔다.
                    Step.iCycle++;
                    return false ;
