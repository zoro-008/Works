    ...
    public class Loader : Part
    {
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                //사이클
                bool isSupply  =  DM.ARAY[ri.LDR].CheckAllStat(cs.None   )  && !IO_GetX(xi.LDR_WorkRackExist) && !IO_GetX(xi.LDR_InArrival    ) ;
                bool isIn      =  DM.ARAY[ri.LDR].CheckAllStat(cs.None   )  && !IO_GetX(xi.LDR_WorkRackExist) &&  IO_GetX(xi.LDR_InArrival    ) ;
