...
{    
    ...
    {   
        ...
        [CategoryAttribute("AxlPara"    ), DisplayNameAttribute("Set Max Position"     )]public double        dSetMaxPos     {get; set;} //모터 최대 표시값 지정.링카운터
    ...
    public class CMotor:IMotor
    {
        ...
        public bool GetServo()//나중에 서보 오프시에 홈시그널 깨지는지 확인해봐야함.
        {
            ...
            //return uiOn != 0;
            bool bRet = false;
            if (Para.eServoPhase == PHASE_2.HighActive)
            {
        ...
        {
            CAXM.AxmMotSetAbsRelMode(Para.iPhysicalNo, _uiAbsRelMode);
        }

        public void LinkJogP(int _iSlvMotrNo, double _dVel, double _dAcc, double _dDec)
        {
            CAXM.AxmMoveVel(Para.iPhysicalNo, +1 * _dVel, _dAcc, _dDec);
            CAXM.AxmMoveVel(_iSlvMotrNo     , -1 * _dVel, _dAcc, _dDec);
        }
        ...
        public bool OverrideVelAtMultiPos(double _dPos, double _dVel, double _dAcc, double _dDec, int _iAraySize, double[] _daOverridePos, double[] _daOverrideVel, int _iTarget, uint _uiOverrideMode)
        {
            uint uiRet = CAXM.AxmOverrideVelAtMultiPos(Para.iPhysicalNo, _dPos, _dVel, _dAcc, _dDec, _iAraySize, _daOverridePos, _daOverrideVel, _iTarget, _uiOverrideMode);
            return uiRet == 0;
        }
        public bool OverrideVelAtPos(double _dPos, double _dVel, double _dAcc, double _dDec, double _dOverridePos, double _dOverrideVel, int _iTarget)
        {
            uint uiRet = CAXM.AxmOverrideVelAtPos(Para.iPhysicalNo, _dPos, _dVel, _dAcc, _dDec, _dOverridePos, _dOverrideVel, _iTarget);
            return uiRet == 0;
        }
    }
}
