...
//      [CategoryAttribute("Motor Para" ), DisplayNameAttribute("Physical No"             )]public int        iPhysicalNo   {get; set;} //실제모터 물리 어드레스.
    ...
    public class CMotrMan
    {
        ...
        public bool OverrideVelAtMultiPos(int _iAxisNo, double _dPos, double[] _daOverridePos, double[] _daOverrideVel)
        {
            double dAcc = UnitToPulse(_iAxisNo, Para[_iAxisNo].dAcceleration) ;
            double dDec = UnitToPulse(_iAxisNo, Para[_iAxisNo].dDeceleration) ;
            double dVel = UnitToPulse(_iAxisNo, Para[_iAxisNo].dRunSpeed)     ;
            int iAraySize       = _daOverridePos.Length;
            int iTarget         = ( int)AXT_MOTION_SELECTION.COMMAND;
            uint uiOverrideMode = (uint)AXT_MOTION_OVERRIDE_MODE.OVERRIDE_POS_START;
            Mtr[_iAxisNo].SetOverrideMaxVel(dVel);
            double dPos = UnitToPulse(_iAxisNo, _dPos);

            double[] dOverridePos = new double[_daOverridePos.Length];
            double[] dOverrideVel = new double[_daOverrideVel.Length];
            for (int i = 0; i < _daOverridePos.Length; i++)
            {
                dOverridePos[i] = UnitToPulse(_iAxisNo, _daOverridePos[i]);
                dOverrideVel[i] = UnitToPulse(_iAxisNo, _daOverrideVel[i]);
            }

            return Mtr[_iAxisNo].OverrideVelAtMultiPos(dPos, dVel, dAcc, dDec, iAraySize, _daOverridePos, _daOverrideVel, iTarget, uiOverrideMode);
        }

        public bool OverrideVelAtPos(int _iAxisNo, double _dPos, double _dOverridePos, double _dOverrideVel)
        {
            double dAcc = UnitToPulse(_iAxisNo, Para[_iAxisNo].dAcceleration);
            double dDec = UnitToPulse(_iAxisNo, Para[_iAxisNo].dDeceleration);
            double dVel = UnitToPulse(_iAxisNo, Para[_iAxisNo].dRunSpeed);
            int iTarget = (int)AXT_MOTION_SELECTION.COMMAND;
            Mtr[_iAxisNo].SetOverrideMaxVel(Para[_iAxisNo].dRunSpeed);
            double dPos = UnitToPulse(_iAxisNo, _dPos);

            
            double dOverridePos = UnitToPulse(_iAxisNo, _dOverridePos);
            double dOverrideVel = UnitToPulse(_iAxisNo, _dOverrideVel);
            

            return Mtr[_iAxisNo].OverrideVelAtPos(dPos, dVel, dAcc, dDec, _dOverridePos, _dOverrideVel, iTarget);
        }

        public double GetRunVel(int _iMotrNo)
        {
            if( _iMotrNo < 0 || _iMotrNo >= m_iMaxMotr) 
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}",_iMotrNo,m_iMaxMotr));
            ...
            return Para[_iMotrNo].dRunSpeed;
        }

        public double GetSlowVel(int _iMotrNo)
        {
            if( _iMotrNo < 0 || _iMotrNo >= m_iMaxMotr) 
            {
                Log.ShowMessageFunc(string.Format("ERR Motr{0} is not Between 0 and {1}",_iMotrNo,m_iMaxMotr));
            ...
            return Para[_iMotrNo].dSlowSpeed;
        }
    }
