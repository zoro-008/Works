...
namespace Machine
{
    public partial class FrameCylinderAPT : Form
    {
        ...
        private void evFwd_Click(object sender, EventArgs e)
        {
            string sText = ((Button)sender).Text;
            Log.Trace(m_sActrName, sText + " Button Clicked",ti.Frm);
            if (SEQ.PRER.CheckSafe(m_iActrId, fb.Fwd , true) && 
                SEQ.PSTR.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.LPCK.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.RPCK.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.VSTG.CheckSafe(m_iActrId, fb.Fwd , true)) ML.CL_Move(m_iActrId, fb.Fwd);
        }
        ...
        private void evBwd_Click(object sender, EventArgs e)
        {
            ...
            if (SEQ.PRER.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.PSTR.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.LPCK.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.RPCK.CheckSafe(m_iActrId, fb.Bwd , true) &&
                SEQ.VSTG.CheckSafe(m_iActrId, fb.Bwd , true)) ML.CL_Move(m_iActrId, fb.Bwd);
