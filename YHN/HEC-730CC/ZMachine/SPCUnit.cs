...
namespace Machine
{
    ...
    public class CLot : DataHandler<TLotData>
    {
        ...
        public void DispLotInfo(ListView _lvLotInfo) //오퍼레이션 창용.
        {
            ...
            //LotId
            _lvLotInfo.Items[0].SubItems[0].Text = "LOT NAME";
            _lvLotInfo.Items[1].SubItems[0].Text = Data.LotNo;
    ...
    public struct TDayData
    {
        ...
        public string sDownTime   ;
        public string sIdleTime   ;
        public string sFailureTime;
    ...
    public class CDay : DataHandler<TDayData>
    {
        ...
        public void Update(EN_SEQ_STAT Stat, string _sCrntDev, bool _bMaint)
        {
            ...
            {
                Data.FailureTime += dTimeGap;
                var Failure = TimeSpan.FromMilliseconds(Data.FailureTime);
                Data.sIdleTime = DateTime.Now.ToString("yyyy") + "/" + Data.LastWrkDay + "||" + Failure.ToString(@"hh\:mm\:ss");
            }
            else if (Stat == EN_SEQ_STAT.Error && ML.ER_GetErrLevel((ei)ML.ER_GetLastErr()) == EN_ERR_LEVEL.Error)
            {
                Data.DownTime += dTimeGap;
                var Down = TimeSpan.FromMilliseconds(Data.DownTime);
                Data.sIdleTime = DateTime.Now.ToString("yyyy") + "/" + Data.LastWrkDay + "||" + Down.ToString(@"hh\:mm\:ss");
            }
            else if (Stat == EN_SEQ_STAT.Running || Stat == EN_SEQ_STAT.RunWarn)
            {
                Data.RunTime += dTimeGap;
                var Run = TimeSpan.FromMilliseconds(Data.RunTime);
                Data.sIdleTime = DateTime.Now.ToString("yyyy") + "/" + Data.LastWrkDay + "||" + Run.ToString(@"hh\:mm\:ss");
            }
            else //Stop Init Warnning WorkEnd 
            {
                Data.IdleTime += dTimeGap;
                var Idle = TimeSpan.FromMilliseconds(Data.IdleTime);
                Data.sIdleTime = DateTime.Now.ToString("yyyy") + "/" + Data.LastWrkDay + "||" + Idle.ToString(@"hh\:mm\:ss");
            }
        ...
        public void DispDayInfo(ListView _lvDayInfo) //오퍼레이션 창용.
        {
            ...
            //Run Time
            //var time = TimeSpan.FromMilliseconds(Data.CycleTime);
            tDateTime = DateTime.FromOADate(OM.EqpStat.dCycleTime);
            _lvDayInfo.Items[6].SubItems[0].Text = "1 CYCLE TIME";
            //_lvDayInfo.Items[7].SubItems[0].Text = time.ToString("hh:mm:ss");
            _lvDayInfo.Items[7].SubItems[0].Text = tDateTime.ToString("mm:ss");
            ////Run Time
            //var time = TimeSpan.FromMilliseconds(Data.RunTime);
            //_lvDayInfo.Items[6].SubItems[0].Text = "RUN TIME";
            ////_lvDayInfo.Items[7].SubItems[0].Text = time.ToString("hh:mm:ss");
            //_lvDayInfo.Items[7].SubItems[0].Text = time.ToString(@"hh\:mm\:ss");
