...
namespace Machine
{
    public partial class FormSPC : Form
    {
        ...
        public void FormInit()
        {
            ...
            dpSttTime.MaxDate = DateTime.Now;
            dpEndTime.MaxDate = DateTime.Now;

            dpSttTime.MinDate = SttDate.AddMonths(-1);
            dpEndTime.MinDate = SttDate.AddMonths(-1);
        ...
        private void sdXLS_FileOk(object sender, CancelEventArgs e)
        {
        ...
        private void btDataView_Click(object sender, EventArgs e)
        {
            ...
            lbUptime.Text = string.Format("Uptime={0:0.0}%",GetLotUptime()) ;

            SPC.ERR.DispData(dpSttTime.Value, dpEndTime.Value, lvError);
            lbMTBA.Text = string.Format("{0:0.0}min",GetMTBA()) ;
            lbMTTA.Text = string.Format("{0:0.0}sec",GetMTTA()) ;

            SPC.FLR.DispData(dpSttTime.Value, dpEndTime.Value, lvFailure);
            lbMTBF.Text = string.Format("{0:0.0}hour",GetMTBF()) ;
            lbMTTR.Text = string.Format("{0:0.0}min" ,GetMTTR()) ;
...
//            SPC.LOT.Data.RunTime       = 
        ...
        private void btLotFind_Click(object sender, EventArgs e)
        {
            //Button SelBtn = sender as Button;
            //int iSel = (int)SelBtn.Tag ;
        ...
        private void btLotDelete_Click(object sender, EventArgs e)
        {
            //Button SelBtn = sender as Button;
            //int iSel = (int)SelBtn.Tag ;
            int iSel = Convert.ToInt32(((Button)sender).Tag);
        ...
        //private void cbUnderRepair_CheckedChanged(object sender, EventArgs e)
        //{
        ...
        {
            btLotDelete .Enabled = SML.FrmLogOn.GetLevel() > EN_LEVEL.Operator ;
            btErrDelete .Enabled = SML.FrmLogOn.GetLevel() > EN_LEVEL.Operator ;
            btFailDelete.Enabled = SML.FrmLogOn.GetLevel() > EN_LEVEL.Operator ;

            btSetRepair .Enabled = SML.FrmLogOn.GetLevel() > EN_LEVEL.Operator ;

          

        }

        private void btSetRepair_Click(object sender, EventArgs e)
        {
            FrmRepair.ShowDialog(this);
            
        }

        private void btLotSave_Click(object sender, EventArgs e)
        {
            SPC.LOT.SaveCsv(lvLot);
        }

        private void btErrSave_Click(object sender, EventArgs e)
        {
            SPC.ERR.SaveCsv(lvError);
        }

        private void btFailSave_Click(object sender, EventArgs e)
        {
