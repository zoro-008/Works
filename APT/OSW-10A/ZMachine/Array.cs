...
namespace Machine
{
    ...
    public class CArrayData
    {
        ...
        public void Load(CConfig Config , bool IsLoad)
        {
            ...
            if(IsLoad) {                                
                ...
                try
                {
                    ...
                    for (int r = 0 ; r < m_iMaxRow ; r++) {                    
                        for (int c = 0 ; c < m_iMaxCol ; c++) {
                            sIndex = string.Format("{0:000}_{1:000}", c , r );
                            Config.GetValue(sArayName , "sUnitID"+sIndex ,out Chip[c,r].sUnitID );
                            Config.GetValue(sArayName , "sDMC1"  +sIndex ,out Chip[c,r].sDMC1   );
                            Config.GetValue(sArayName , "sDMC2"  +sIndex ,out Chip[c,r].sDMC2   );
                            Config.GetValue(sArayName , "Query"  +sIndex ,out Chip[c,r].sQuery  );
                        }                    
                    }     
                }
                catch(Exception e)
                {
                    ...
                    Log.ShowMessage("Array Loading Exception",e.Message);
                ...
                for (int r = 0 ; r < m_iMaxRow ; r++) {                    
                    for (int c = 0 ; c < m_iMaxCol ; c++) {
                        sIndex = string.Format("{0:000}_{1:000}", c , r );
                        Config.SetValue(sArayName , "sUnitID"+sIndex ,Chip[c,r].sUnitID );
                        Config.SetValue(sArayName , "sDMC1"  +sIndex ,Chip[c,r].sDMC1   );
                        Config.SetValue(sArayName , "sDMC2"  +sIndex ,Chip[c,r].sDMC2   );
                        Config.SetValue(sArayName , "Query"  +sIndex ,Chip[c,r].sQuery  );
                    }                    
                }                
            } 
        }
        ...
        //public int GetCntStat(params object[] p) { 
        //    int iCnt = 0;
        //    for (int i = 0; i < p.Length; i++)
        //    {
        //        iCnt += GetCntStat((cs)p[i]);
        //    }                
        //    return iCnt; 
        //}
