...

{
    ...
    public class MotrPstn
    {
        ...
        public int iMotrMosCellSize = 20; //모터 포지션 셀높이
        //ListView Column 사이즈
        ...
        public MotrPstn(uint _uMotrId , uint _uPstnCnt)
        {
            ...
            lvPstnView.Columns.Add("Go"    , iGoWidth    , HorizontalAlignment.Left);//55
            lvPstnView.Columns.Add("Input" , iInputWidth , HorizontalAlignment.Left);
            ...
            for (int i = 0; i < _uPstnCnt; i++)
            {
                ...
                lvPstnView.Items[i].SubItems[0].BackColor = Color.LightGray  ;
        ...
        private void btGoClick(object sender, EventArgs e)
		{
            ...
            if(m_uMotrId == (int)mi.LODR_ZLift) bRet = SEQ.LODR.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.TOOL_XRjct) bRet = SEQ.TOOL.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.IDXR_XRear) bRet = SEQ.IDXR.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.IDXF_XFrnt) bRet = SEQ.IDXF.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.TOOL_YTool) bRet = SEQ.TOOL.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.TOOL_ZPckr) bRet = SEQ.TOOL.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.BARZ_XPckr) bRet = SEQ.BARZ.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.BARZ_ZPckr) bRet = SEQ.BARZ.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.STCK_ZStck) bRet = SEQ.STCK.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
            if(m_uMotrId == (int)mi.TOOL_ZVisn) bRet = SEQ.TOOL.CheckSafe((mi)m_uMotrId , (pv)uPstnNo);
        ...
        private void btInputClick(object sender, EventArgs e)
		{
            ...
            double dCmdPos = GetCmdPos((int)m_uMotrId);
            string sCmdPos = dCmdPos.ToString("N4");
            double dInputPos = double.Parse(sCmdPos);
        ...
        {
            if(PstnValue.Length <= _uPstnNo) return ;
            ...
            if (lvPstnView.Parent != null)
            {
                ...
                if (lvPstnView.InvokeRequired)
                {
                    lvPstnView.Invoke(new MethodInvoker(delegate()
                    {
                        ...
                        lvPstnView.Items[(int)_uPstnNo].SubItems[0].BackColor = _bCommon ? Color.LightSkyBlue : Color.LightGray ; //Color.Silver;
                    }));
                }
                ...
                else
                {
                    ...
                    btPstnInput[_uPstnNo].Visible = !_bFix && !_bOffset;
                    lvPstnView.Items[(int)_uPstnNo].SubItems[0].Text = _sName;
                    lvPstnView.Items[(int)_uPstnNo].SubItems[0].BackColor = _bCommon ? Color.LightSkyBlue : Color.LightGray ;
                }
        ...
        public void SetListviewParent(Panel _pnBase)
        {
            ...
            for (int i = 0; i < m_uMaxPstn; i++)
            {
                ...
                lvPstnView.Items[(int)i].SubItems[0].BackColor = PstnValue[(int)i].bCommon ? Color.LightSkyBlue : Color.LightGray ;
    ...
    public static class PM
    {
        ...
        {
            MotrPstn[_uAxisNo].SetProp(_uPstnNo , _sName.Trim() , _bOffset , _bFix , _bCommon);
        }
        ...
        {
            int iRet = MotrPstn[0].iNameWidth + MotrPstn[0].iValueWidth + MotrPstn[0].iGoWidth + MotrPstn[0].iInputWidth;
            return iRet;
        }
    }
