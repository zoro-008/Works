...
namespace COMMON
{
    ...
    public class RS232_110Xi4
    {
        ...
        //보내는 메세지임.
        public void PrintBar2(int _iYOffset , 
                              string _sTrayLabel_GroupingNo, string _sTrayLabel_MaterialNo, string _sTrayLabel_BatchNo,  string _sSecurityCode,  //여기까지는 바코드 내용.
                              string _sTrayInfomation_DeviceNumber , string _sLotTraveler_LotAlias, string _sTrayInfomation_Bin, string _sToff)
        {
            ...
            string sBarcode = _sTrayLabel_GroupingNo + _sTrayLabel_MaterialNo + _sTrayLabel_BatchNo + _sSecurityCode ;
            string sTemp = "";
            sTemp += @"^XA^LH0," + _iYOffset.ToString(); //^XA=StartFormat ^LH = 0,0 좌표의 절대값을 설정.전체포지션이 이동된다.
            sTemp += "^^BY2,3,80^FT50,82^BCN,,N,N"; //^BY=최고좁은바의 넓이가2dots이고 넓은바와 좁은바의 비율이 3.0유닛이다 ^BC=Code128포멧
            sTemp += @"^FD>:";//^FD= Start of Field data for bar code.
            sTemp += sBarcode + "^FS";
            sTemp += @"^FT20,100^A0N,21,21^FH\^FDMATERIAL:";  //^FT=문장의 시작위치 Y축은 밑쪽이다.  ^A0N=디폴트폰트셀렉트
//            sTemp += @"^FT95,100^A0N,21,21^FH\^FD";
            sTemp += _sTrayLabel_MaterialNo + "^FS";
            sTemp += @"^FT22,0^A0N,21,21^FH\^FDDeviceNo:";
            sTemp += _sTrayInfomation_DeviceNumber + "^FS";
            sTemp += @"^FT394,0^A0N,21,21^FH\^FDGroup:";
            sTemp += _sLotTraveler_LotAlias + "^FS";
            sTemp += @"^FT295,100^A0N,21,21^FH\^FDBATCH:";
            sTemp += _sTrayLabel_BatchNo + "^FS";
            sTemp += @"^FT580,100^A0N,20,19^FH\^FDBIN:";
            sTemp += _sTrayInfomation_Bin + "^FS";
            sTemp += @"^PQ1,0,1,Y^XZ";
            sTemp += "~TA"; // command lets you adjust the rest position of the media after a label is printed, which changes the position at which the label is torn or cut.
            sTemp += _sToff;
        ...
        public bool Update()
        {
            ...
            switch (iStep)
            {
                ...
                case 11: 
                    //접속이 끊어졌을때 사용하는건데. 문제가 있을수 있다. 확인 부탁
                    //JS
                    if (Delay.OnDelay(1000))
                    {
                        //끊어졌으면 리커넥션 동작을 위해 100번으로
                        iStep=100 ;
                        return false ;               
                    }
