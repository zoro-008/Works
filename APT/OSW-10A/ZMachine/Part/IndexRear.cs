    ...
    public class IndexRear : Part
    {
        ...
        override public bool ToStart() //스타트를 하기 위한 함수.
        {
            ...
            switch (Step.iToStart)
            {
                ...
                case 10:
                    ...
                    {
                        MoveCyl(ci.IDXR_ClampClOp, fb.Bwd);
                        Step.iToStart++;
                        return false;
                    }
                    Step.iToStart = 0;
                ...
                case 11:
                    if (!CL_Complete(ci.IDXR_ClampClOp, fb.Bwd)) return false;
                    MoveCyl(ci.IDXR_ClampUpDn, fb.Bwd);
                    Step.iToStart++;
                    return false;
        ...
        override public bool ToStop() //스탑을 하기 위한 함수.
        {
            ...
            switch (Step.iToStop)
            {
                ...
                case 10:
                    OM.EqpStat.dLastIDXRPos = SM.MT_GetCmdPos(mi.IDXR_XRear);
                    Step.iToStop++;
                    return false;
        ...
        override public bool Autorun() //오토런닝시에 계속 타는 함수.
        {
            ...
            if (Step.eSeq == sc.Idle)
            {
                ...
                bool isCycleGet     = DM.ARAY[ri.SPLR].CheckAllStat(cs.Unknown) && DM.ARAY[ri.IDXR].CheckAllStat(cs.None) && SEQ.IDXF.GetStep().eSeq != IndexFront.sc.Get && !OM.CmnOptn.bIdxRSkip;
                bool isCycleBarcode = DM.ARAY[ri.IDXR].CheckAllStat(cs.Unknown) && 
                                    // (DM.ARAY[ri.IDXF].CheckAllStat(cs.None)    ||  SEQ.IDXF.GetStep().eSeq == IndexFront.sc.Out );
                                     (DM.ARAY[ri.IDXF].CheckAllStat(cs.None)    ||  (!IO_GetX(xi.RAIL_TrayDtct2 ) && SEQ.IDXF.GetStep().eSeq == IndexFront.sc.Out)) ;
                                     
                bool isCycleOut     = DM.ARAY[ri.IDXR].CheckAllStat(cs.Good)|| DM.ARAY[ri.IDXR].CheckAllStat(cs.Empty) && DM.ARAY[ri.OUTZ].CheckAllStat(cs.None) && (DM.ARAY[ri.IDXF].CheckAllStat(cs.None) || DM.ARAY[ri.IDXF].CheckAllStat(cs.Unknown)) && !OM.CmnOptn.bGoldenTray;// JS BarCodeData Add.);
                bool isCycleEnd     = (DM.ARAY[ri.IDXR].CheckAllStat(cs.None) || (!DM.ARAY[ri.IDXR].CheckAllStat(cs.None) && OM.CmnOptn.bGoldenTray)) ;//&& LOT.LotList.Count == 0;
                  
                //여러모로 문제가 많아서 스킵 하고 사이클시작할때 한번씩 본다.        
                //if(isCycleGet || isCycleBarcode || isCycleOut) {
                //    if( DM.ARAY[ri.IDXR].CheckAllStat(cs.None) && IO_GetX(xi.IDXR_TrayDtct) ) {ER_SetErr(ei.PKG_Unknwn , "RearIndex Unknwn PKG Error."   ); return false;}
        ...
        public bool FindChip(out int _iC, out int _iR,out cs _iChipStat , out bool _bFlying , bool _bGldTry = false) 
        {
            ...
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.Vision  ,ref ColVision , ref RowVision);
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.Empty   ,ref ColEmpty  , ref RowEmpty );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG0     , ref ColNG0   , ref RowNG0   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG1     , ref ColNG1   , ref RowNG1   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG2     , ref ColNG2   , ref RowNG2   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG3     , ref ColNG3   , ref RowNG3   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG4     , ref ColNG4   , ref RowNG4   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG5     , ref ColNG5   , ref RowNG5   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG6     , ref ColNG6   , ref RowNG6   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG7     , ref ColNG7   , ref RowNG7   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG8     , ref ColNG8   , ref RowNG8   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG9     , ref ColNG9   , ref RowNG9   );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.NG10    , ref ColNG10  , ref RowNG10  );
            DM.ARAY[ri.IDXR].FindLastColFrstRow(cs.Unknown , ref ColUkwn  , ref RowUkwn  );

            //순서 중요 하다. 
            //장비전면에서 봤을때 트레이 오른쪽부터 같은 Col에서는 장비 전면쪽 부터 작업을 하는데 
            ...
            _iChipStat = DM.ARAY[ri.IDXR].GetStat(_iC , _iR);
            _bFlying = DM.ARAY[ri.IDXR].GetCntColStat(_iC, cs.Vision) != 0;
        ...
        public bool CycleHome()
        {
            ...
            switch (Step.iHome) {
                ...
                case 13:
                    if (!MT_GetHomeDone(mi.IDXR_XRear)) return false;
                ...
                case 15:
                    if (!MT_GetStop(mi.IDXR_XRear)) return false;
                ...
                case 16:
                    if (!CL_Complete(ci.IDXR_ClampUpDn, fb.Fwd)) return false;
                    CL_Move(ci.IDXR_ClampClOp, fb.Fwd);
                    Step.iHome++;
                    return false;

                case 17:
                    if (!CL_Complete(ci.IDXR_ClampClOp, fb.Fwd)) return false;
                    Step.iHome = 0;
                    return true;
                ...
                case 51:
                    if(!MT_GetStopInpos(mi.IDXR_XRear))return false;
                    Step.iHome = 0;
                    return true;
        ...
        public bool CycleGet()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 12:
                    if (!CL_Complete(ci.IDXR_ClampUpDn, fb.Bwd)) return false;
                    MoveMotr(mi.IDXR_XRear, pv.IDXR_XRearClamp);
                    MoveCyl(ci.IDXR_ClampClOp, fb.Fwd);
                    Step.iCycle++;
                    return false;

                case 13:
                    if (!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearClamp)) return false;
                    if (!CL_Complete(ci.IDXR_ClampClOp, fb.Fwd)) return false;
                    MoveCyl(ci.IDXR_ClampClOp, fb.Bwd);
                    Step.iCycle++;
                    return false;
                ...
                case 14:
                    if (!CL_Complete(ci.IDXR_ClampClOp, fb.Bwd)) return false;
                    MoveCyl(ci.IDXR_ClampUpDn, fb.Fwd);
                    Step.iCycle++;
                    return false;

                case 15:
                    if (!CL_Complete(ci.IDXR_ClampUpDn, fb.Fwd)) return false;
                    //MoveMotr(mi.IDXR_XRear, pv.IDXR_XRearClamp, 0);
                    Step.iCycle++;
                    return false;

                case 16:
                    //if (!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearClamp, 0)) return false;
                ...
                case 17:
                    ...
                    DM.ARAY[ri.IDXR].SetStat(cs.Unknown);
                    for (int c = 0; c < OM.DevInfo.iTRAY_PcktCntX; c++)
                    {
                        for (int r = 0; r < OM.DevInfo.iTRAY_PcktCntY; r++)
                        {
                            ...
                            {
                                DM.ARAY[ri.IDXR].SetStat(c, r, cs.None);
                            }
                        }
                    }
        ...
        public bool CycleBarcode()
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 12:
                    if (!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearBarcode)) return false;
                ...
                case 17:
                    if(!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearVsnStt1)) return false;
        ...
        public bool CycleOut() 
        {
            ...
            switch (Step.iCycle)
            {
                ...
                case 10:
                    ...
                    MoveMotr(mi.IDXR_XRear, pv.IDXR_XRearUld);
                    if (!OM.CmnOptn.bOracleNotUse&& !OM.CmnOptn.bIdleRun) { 
                       if(DM.ARAY[ri.IDXR].CheckAllStat(cs.Good)){
                           if (!OM.CmnOptn.bOracleNotWriteInsp) { 
                               //for (int r = 0; r < DM.ARAY[ri.IDXR].GetMaxRow(); r++)
                               //{
                               //    for (int c = 0; c < DM.ARAY[ri.IDXR].GetMaxCol(); c++) { 
                               //        if (!SEQ.Oracle.Insert("insert " + DM.ARAY[ri.IDXR].Chip[c,r].sQuery))
                               //        {
                               //            ER_SetErr(ei.ETC_Oracle , SEQ.Oracle.GetLastMsg());
                               //        }
                               //    }
                               //}
                               for (int r = DM.ARAY[ri.IDXR].GetMaxRow()-1; r >= 0 ; r--)
                               {
                                   for (int c = 0; c < DM.ARAY[ri.IDXR].GetMaxCol(); c++) { 
                                       if (!SEQ.Oracle.Insert("insert " + DM.ARAY[ri.IDXR].Chip[c,r].sQuery))
                                       {
                           ...
                           //if (!SEQ.Oracle.SendUnitInspectionQueryOneByOne())
                           //{
                           //    ER_SetErr(ei.ETC_Oracle , SEQ.Oracle.GetLastMsg());
                           //}
                ...
                case 11:
                    if (!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearUld)) return false;
                ...
                case 13:
                    if (!CL_Complete(ci.IDXR_ClampUpDn, fb.Bwd)) return false;
                    MoveMotr(mi.IDXR_XRear, pv.IDXR_XRearWait);
                ...
                case 14:
                    if (!MT_GetStopPos(mi.IDXR_XRear, pv.IDXR_XRearWait)) return false;
                    Step.iCycle++;
                    return false;
