...
//      [CategoryAttribute("Motor Para" ), DisplayNameAttribute("Physical No"             )]public int        iPhysicalNo   {get; set;} //실제모터 물리 어드레스.
        ...
        [CategoryAttribute("Speed Para" ), DisplayNameAttribute("Speed Ratio"             )]public double     dSpeedRatio   {get; set;} //전체스피드에 대한 비율. 홈빼고 Run Slow Manual 
    ...
    {
        public struct TParaAll//모든 모터에 적용 되는 파라.
        {
            public double SpeedRatioAll ;  //AP텍 전용 이상하니 다른프로젝트는 빼자.
        }
        public TParaAll         ParaAll ;
        ...
        public bool LoadSaveAll(bool _bLoad)
        {
            ...
            if (_bLoad)
            {
                ...
                for (int i = 0; i < m_iMaxMotr; i++)
                {
                    ...
                    //Special Handling
                         if(Para[i].dSpeedRatio == 0  ){Para[i].dSpeedRatio= 100 ;}
                    else if(Para[i].dSpeedRatio <  10 ){Para[i].dSpeedRatio= 10  ;}
                    else if(Para[i].dSpeedRatio >  100){Para[i].dSpeedRatio= 100 ;}
                }                
            }
            else
            {
        ...
        protected void Go(int _iMotrNo, double _dPos, double _dVel, double _dAcc, double _dDec)
        {
            ...
            double dAcc = UnitToPulse(_iMotrNo, _dAcc) * Para[_iMotrNo].dSpeedRatio/100.0 * ParaAll.SpeedRatioAll / 100.0;
        ...
        {
            //축이 여러게일 경우 비율은 마스터꺼로.
            double dVel = _dVel * Para[_iMotrPhyAdds[0]].dSpeedRatio/100.0 * ParaAll.SpeedRatioAll / 100.0;;
            ...
            {
                Mtr[_iMotrPhyAdds[0]].GoMultiAbs(_iMotrPhyAdds, _dPoses, UnitToPulse(_iMotrPhyAdds[0], dVel), 
        ...
        {
            //double dPos = UnitToPulse(_iMotrNo, _dPos) ;
            double dVel = UnitToPulse(_iMotrNo, _dVel) * Para[_iMotrNo].dSpeedRatio /100.0 * ParaAll.SpeedRatioAll / 100.0;
        ...
        //요밑에 것들은 LineMove기반으로 다시 검증해야 할듯 UnitToPul가 안들어갔음.
