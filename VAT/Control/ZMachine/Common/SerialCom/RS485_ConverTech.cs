...
namespace Machine
{
    ...
    public class RS485_ConverTech
    {
        ...
        private void DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            bool bRet1 = sRecvedMsg.Split(cETX).Length >= 2 && sRecvedMsg.Split(cSTX).Length >= 2 && sRecvedMsg.IndexOf("@A@@") >= 0; //A Ack
            bool bRet2 = sRecvedMsg.Split(cETX).Length >= 2 && sRecvedMsg.Split(cSTX).Length >= 2 && sRecvedMsg.IndexOf("AA@@") >= 0; //A Ack
            bool bRet3 = sRecvedMsg.Split(cETX).Length >= 2 && sRecvedMsg.Split(cSTX).Length >= 2 && sRecvedMsg.IndexOf("BA@@") >= 0; //A Ack
            bool bRet4 = sRecvedMsg.IndexOf(cSTX)      >= 0 && sRecvedMsg.IndexOf(cACK ) >= 0 ; //A Ack

            int i1 = sRecvedMsg.Split(cETX).Length - 1;

            if (!bRet1 && !bRet2 && !bRet3 && !bRet4) return;
         
            

            if(PreStat[iPreId].sRsv != sRecvedMsg)
            {
                Log.Trace(sRecvedMsg,LOG_ID);
            }
            PreStat[iPreId].sRsv = sRecvedMsg ;
            ...
            //전체상태 가져오는 것의 응답.
            //CommentA
            if (sRecvedMsg.Length == 21)
            {
                string sEcho = sRecvedMsg.Substring(sRecvedMsg.IndexOf(cSTX) + 1, 1);
                ...
                sEcho = sRecvedMsg.Remove(sRecvedMsg.IndexOf(cSTX) , sRecvedMsg.IndexOf(cETX) - sRecvedMsg.IndexOf(cSTX) + 1);
