...
namespace SML
{
    public class CDioMan
    {
        ...
        public bool Init(EN_LAN_SEL _eLanSel,string _sParaFolderPath,EN_DIO_SEL _eDioSel,Enum _eX,Enum _eY)
        {
            ...
            //초기 귀찮음에 넣엇다 뺏다 해야 하나.
            //for(int i=0; i<m_iMaxIn; i++) { 
            //    m_aIn[i].Para.sName = arrayTemp1.GetValue(i).ToString();
            //    m_aIn[i].Para.SComt = arrayTemp1.GetValue(i).ToString();
            //    m_aIn[i].Para.iAdd  = i;
            //}
            //for(int i=0; i<m_iMaxOut; i++) { 
            //    m_aOut[i].Para.sName = arrayTemp2.GetValue(i).ToString();
            //    m_aOut[i].Para.SComt = arrayTemp2.GetValue(i).ToString();
