...
namespace Machine
{
    ...
    public sealed class SEQ
    {
        ...
        //static public PControl PCON = new PControl        ((int)pi.PCON + ti.Max);
        //static public Part[] m_Part = new Part[(int)pi.MAX_PART];

        static public RS485_ConverTech        ConverTech = new RS485_ConverTech       (1, "ConverTech" );           
        static public RS232_Daegyum_Seasoning Daegyum    = new RS232_Daegyum_Seasoning(2, "Daegyum"    );           

        static public object lockObject = new object();
        static public Aging aging = new Aging((int)pi.PCON + ti.Max);
        ...
        public static void Init()
        {
            ...
            //m_Part[(int)pi.PCON] = PCON; //m_Part[(int)pi.LODR].SetPartId((int)pi.LODR + ti.Max);
        ...
        private static void UpdateButton  ()
        {
            ...
            bool bHoldSw    = ML.IO_GetXUp(xi.HoldSw ) ;           
            bool bManualSw  = ML.IO_GetX  (xi.ManualSw);
            bool bAutoSw    = ML.IO_GetX  (xi.AutoSw  );
            
            //bool bDoorOpen1 = ML.IO_GetX(xi.Door1)          ||  ML.IO_GetX(xi.Door2)           ;
            bool bDoorOpen2 = ML.IO_GetX(xi.ShieldDoorOpen) || !ML.IO_GetX(xi.ShieldDoorClose) ;
            if(bManualSw)
            {
                ML.IO_SetY(yi.PowerManual,true);
                //if(m_bRun || bDoorOpen1 || bDoorOpen2) {
                if(m_bRun || bDoorOpen2) {
                    ML.ER_SetErr(ei.ETC_Manual , "Can not change to manual mode" );
                    ML.IO_SetY(yi.AnodeOn,true);
                    ML.IO_SetY(yi.FocusOn,true);
                    ML.IO_SetY(yi.GateOn ,true);
                }
                else                                   {
                    ML.IO_SetY(yi.AnodeOn,true);
                    ML.IO_SetY(yi.FocusOn,true);
            ...
            if(bAutoSw)
            {
                ML.IO_SetY(yi.PowerManual,false);
            }
            ...
            ML.IO_SetY(yi.StopLamp , ML.IO_GetX(xi.StopSw ) || !m_bRun || bStopBtnFlick            );
            ML.IO_SetY(yi.ResetLamp, ML.IO_GetX(xi.ResetSw) || (m_bFlick && isErr)                 );
            ML.IO_SetY(yi.HoldLamp , ML.IO_GetX(xi.HoldSw )                                        );
            ...
            if (bStartSw)
            {
                ...
                if(bManualSw             ) { Log.ShowMessage("Error" , "Auto Switch로 변경해 주세요."  ); bStartSw = false ; }
                ...
                //FormDevice.FrmDeviceSet.bComplete = false;
                //FormDevice.FrmDeviceSet.RecipeCheck();

                //m_tmTemp.Clear();
                //while(!FormDevice.FrmDeviceSet.bComplete)
                //{
                //    System.Windows.Forms.Application.DoEvents();
                //    if (m_tmTemp.OnDelay(true, 1000))
                //    {
                //        
                //        Log.ShowMessage("Error" , "RECIPE 구성을 체크해 주세요"); bStartSw = false ; 
                //        break;
                //    }
                //}
                //if() { Log.ShowMessage("Error" , "RECIPE 구성을 체크해 주세요"); bStartSw = false ; }
            ...
            if (bResetSw)
            {
                if (!m_bRun && !ML.ER_IsErr())
                {
                    ConverTech.SendE(0); 
                    ConverTech.SendE(1); 
                    ConverTech.SendE(2); 
                    Daegyum.SendOutOnOff(false);
                    ConverTech.SendA(0); 
                    ConverTech.SendA(1); 
                    ConverTech.SendA(2); 
                }
            ...
            if (m_tmToStop.OnDelay(m_iStep == EN_SEQ_STEP.ToStop, 30000))      //  20000)) {
            {
                //Trace Log.
                string Msg;
        ...
        public static bool  InspectEmergency()
        {
            ...
            //Check Emergency
            //if(ML.IO_GetXUp(xi.EmgSw1) || ML.IO_GetXUp(xi.EmgSw2))
            //{
            //    ConverTech.SendE(0); 
            //    ConverTech.SendE(1); 
            //    ConverTech.SendE(2); 
            ...
            //    ConverTech.SendA(2); 
            //}
            if(ML.IO_GetX(xi.EmgSw1) || ML.IO_GetX(xi.EmgSw2))
            {
                //if(ConverTech.Stat[0].bOutput) ConverTech.SendE(0); 
                //if(ConverTech.Stat[1].bOutput) ConverTech.SendE(1); 
                //if(ConverTech.Stat[2].bOutput) ConverTech.SendE(2); 
                //통신에러 보기 안좋아서 이걸로 대체
                ML.IO_SetY(yi.AnodeOn,false);
                ML.IO_SetY(yi.FocusOn,false);
                ML.IO_SetY(yi.GateOn ,false);

                ML.IO_SetY(yi.CathodeGND ,false);
                ML.IO_SetY(yi.CathodeGND ,false);
                ML.IO_SetY(yi.CathodeSWT ,false);
                ML.IO_SetY(yi.FocusGND   ,false);
                ML.IO_SetY(yi.FocusPower ,false);
                ML.IO_SetY(yi.GateGND    ,false);
                ML.IO_SetY(yi.GatePower  ,false);
        ...
        public static bool  InspectLive()
        {
            bool isOk = true ;
        ...
        public static bool  InspectDoor()
        {
            ...
            //bool Door1 = SEQ._bFlick && (!ML.IO_GetX(xi.Door1)           || !ML.IO_GetX(xi.Door2));
            //bool Door2 = SEQ._bFlick && (!ML.IO_GetX(xi.ShieldDoorClose) ||  ML.IO_GetX(xi.ShieldDoorOpen));

            if(ML.IO_GetX(xi.ShieldDoorClose))
            {
                if(ML.IO_GetX(xi.HoldSw)) {
                    if(m_iSeqStat == EN_SEQ_STAT.Stop) ML.IO_SetY(yi.ShieldUnLock,true );
                }
                else ML.IO_SetY(yi.ShieldUnLock,false);
            }

            if(m_iStep == EN_SEQ_STEP.Run){
                //if( ML.IO_GetX(xi.Door1          )){ML.ER_SetErr(ei.ETC_Door      , "Front Door Left Sensor"     );isOk= false;}
                //if( ML.IO_GetX(xi.Door2          )){ML.ER_SetErr(ei.ETC_Door      , "Front Door Right Sensor"    );isOk= false;}
                if( ML.IO_GetX(xi.ShieldDoorOpen )){ML.ER_SetErr(ei.ETC_ShieldDoor, "Shield Door Open"           );isOk= false;}
                if(!ML.IO_GetX(xi.ShieldDoorClose)){ML.ER_SetErr(ei.ETC_ShieldDoor, "Shield Door Is Not Closed"  );isOk= false;}

            }

            bool bShieldOpen = !ML.IO_GetX(xi.ShieldDoorClose) || ML.IO_GetX(xi.ShieldDoorOpen);
            if(bShieldOpen || !isOk)
            {
        ...
        {
            //if(!aging.CheckList(0)) return false;
            //if(!aging.CheckList(1)) return false;
            //if(!aging.CheckList(2)) return false;

            if(!aging.StartCon ( )) return false;
        ...
        public static void UpdateSeqState() 
        {
            ...
            bool isManual  =  ML.IO_GetX(xi.ManualSw);//MM.GetManNo() > mc.AllHome;
            ...
            if (m_iSeqStat != iPreStat)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    switch (iStat)
                    {
                        ...
                        case EN_SEQ_STAT.ToStart: sStatText = "STARTING"+ sSub; break;
                        case EN_SEQ_STAT.ToStop : sStatText = "STOPING" + sSub; break;
